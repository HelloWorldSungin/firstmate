...
#   fm-afk-return.sh quiet-off  Explicit attended quiet cleanup, with no away return.
...
return_guard() {
  local reasons
  if fm_afk_contract_present "$STATE" || { [ -e "$STATE/.afk" ] && [ "$(fm_classify_afk_mode "$STATE")" != quiet ]; }; then
...
main() {
  ...
  case "$mode" in
    quiet-off)
      # Explicit attended exit uses the same daemon teardown, without an away
      # brief, archive, or catch-up gate. Never consume an actual away record.
      if fm_afk_contract_present "$STATE" || [ -e "$GATE" ] || {
        [ -e "$STATE/.afk" ] && [ "$(fm_classify_afk_mode "$STATE")" != quiet ];
      }; then
        printf 'fm-afk-return: quiet-off cannot clear an away lifecycle; use the ordinary return path\n' >&2
        return 3
      fi
      "$SCRIPT_DIR/fm-afk-launch.sh" stop
      return
      ;;
    begin|check)
      if [ -e "$STATE/.afk" ] && [ "$(fm_classify_afk_mode "$STATE")" = quiet ] && ! fm_afk_contract_present "$STATE"; then
        printf 'fm-afk-return: quiet mode remains active; use quiet-off only for an explicit quiet exit\n' >&2
        return 3
      fi
      ;;
