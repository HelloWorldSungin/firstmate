...
# filled in. Ship, design, and scout `# Task` sections have two subsections Firstmate
...
#   The flag must be explicit because {TASK} and {FIRSTMATE_SPEC} are filled after scaffolding and the
...
# fm-dod-lib.sh to every ship/design/scout launch brief, so this file never becomes a
...
${FIRSTMATE_REPO_CREW_SECTION}$TASK_SECTION
...
EOF
echo "scaffolded: $BRIEF ($OUTPUT_KIND, mode=$MODE; replace {TASK} and {FIRSTMATE_SPEC})"
