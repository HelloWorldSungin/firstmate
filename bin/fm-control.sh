...
verify_interrupt_running() {
  ...
  if fm_control_backend_state_verified "$BACKEND"; then
    ...
    # Quoted because the dash makes this literal read as arithmetic to
    # ShellCheck's dataflow analysis; the value itself is the reported proof
    # token and must stay `agent-alive`.
    proof='agent-alive'
  fi
  printf '%s' "$proof"
}
