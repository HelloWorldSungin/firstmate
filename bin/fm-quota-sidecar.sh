...
# file, wrong schema, stale observation, future timestamp, or producer error is
# UNKNOWN and exits 0.
#
# Output is one fm-quota-sidecar-reader.v1 JSON document. CURRENT records expose
# `windows`; UNKNOWN records expose the same safe projection only as
# `last_known_windows`. The projection deliberately excludes labels, source,
...
FRESHNESS=${FM_QUOTA_SIDECAR_FRESHNESS_SECONDS:-7200}
case "$FRESHNESS" in
  ''|*[!0-9]*|0*) FRESHNESS=7200 ;;
esac
...
case "$FRESHNESS" in
  ''|*[!0-9]*|0*) die_usage "freshness seconds must be a positive integer" ;;
esac
...
emit_document() { # <provider>, validated document on stdin
  jq -c --arg provider "$1" --argjson now "$NOW" --argjson freshness "$FRESHNESS" '
    ...
    (if .status == "error" then ["UNKNOWN", "source_error"]
     elif $captured_age < 0 or $attempt_age < 0 then ["UNKNOWN", "future_timestamp"]
     elif $attempt_epoch < $captured_epoch then ["UNKNOWN", "invalid_timestamp_order"]
     elif $captured_age > $freshness or $attempt_age > $freshness then ["UNKNOWN", "stale"]
     else ["CURRENT", "fresh"]
