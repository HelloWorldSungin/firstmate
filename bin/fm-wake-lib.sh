...
fm_firstmate_root_home() {
  ...
  while [ -e "$home/.fm-secondmate-parent" ] || [ -L "$home/.fm-secondmate-parent" ]; do
    ...
    # A remote parent lives on another filesystem. This home is the root of
    # the locally knowable tree, so its descendants share its project locks.
    # Parsing above still refuses malformed bindings instead of guessing.
    [ "$FM_SECONDMATE_PARENT_ROUTE" != remote ] || break
