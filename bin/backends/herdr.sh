...
#
# Cursor is one of those other agents, and its native agent-state is never
# legibly idle (measured live: herdr reports a cursor pane `blocked` in every
# state - idle, mid-turn, and after - so the idle-baseline path above is
# structurally unreachable for it). It therefore always lands in the composer
# branch, and cursor's mid-turn composer row renders its own placeholder beside a
