...
fm_backend_herdr_agent_prompt_capability_check() {  # <session>
  local session=$1 out code
  fm_backend_herdr_server_ensure "$session" || return 1
  if out=$(fm_backend_herdr_cli "$session" agent prompt "__firstmate_prompt_capability__" "" 2>&1); then
    echo "error: the Herdr server for session '$session' returned an invalid success for the atomic prompt capability probe" >&2
    return 1
  fi
  code=$(printf '%s' "$out" | jq -r '.error.code // empty' 2>/dev/null || true)
  [ "$code" = empty_agent_prompt ] && return 0
  echo "error: cursor and agy require agent.prompt on the Herdr server for session '$session' (probe returned ${code:-an unreadable response}); restart or update that server before using this harness" >&2
  return 1
}
...
fm_backend_herdr_pane_agent_free_sample() {  # <session> <pane-id> [process-info-json]
...
#
#   agent      - a foreground process has a verified harness identity through
#                the shared classifier.
#   shell      - the exact process-info sample satisfies the terminal-wide
#                absence proof owned by fm_backend_herdr_pane_agent_free_proof.
#   other      - readable foreground or process-tree evidence cannot prove
#                absence. This includes background agents, unknown processes,
#                and an empty foreground during exec handoff. Retry within the
#                bounded settle window to let transient prompt helpers finish.
#   unreadable - process-info failed or did not identify this pane and shell;
#                an empty foreground naming a vanished shell also fails closed.
#
...
fm_backend_herdr_pane_process_state_sample() {  # <session> <pane_id>
  ...
  count=$(printf '%s' "$info" | jq -er \
    ...
    || { printf 'unreadable'; return 0; }
  if [ "$count" -eq 0 ]; then
    # An exec handoff may temporarily omit the foreground. Retry a real shell,
    # but never infer absence from a provider naming a vanished process.
    if ! "${FM_HERDR_PS_BIN:-ps}" -axo pid=,ppid=,stat=,comm=,tty= 2>/dev/null \
      | awk -v want="$shell_pid" '$1 == want { found=1 } END { exit !found }'; then
      printf 'unreadable'
    else
      printf 'other'
    fi
  ...
  # Positive harness identity is shared with upstream. Agent absence must also
  # satisfy the fork's terminal-wide proof, including orphaned same-TTY jobs,
  # suspended agents, and the nested Treehouse shell chain. Reuse this exact
  # process-info sample so the proof and identity view cannot drift apart.
  if [ "$others" -eq 0 ] && fm_backend_herdr_pane_agent_free_sample "$session" "$pane_id" "$info"; then
    printf 'shell'
  else
    printf 'other'
  fi
...
# versioned evidence.
#
# The shared process classifier uses fm_backend_herdr_pane_agent_free_sample
# for absence, preserving the terminal-wide proof owned by
# fm_backend_herdr_pane_agent_free_proof. Only positive absence unlocks recovery;
# a failed process-info read is unreadable, and an inconclusive readable sample
# remains alive. Neither state licenses removal or recovery.
#
...
# agy uses Herdr's atomic agent prompt operation and requires the exact input to
# appear in the native session transcript. Other agents retain the send-text,
# Enter, and transition-confirmation path below.
#
...
# not receive this conversion, and neither does a pane that was ALREADY blocked
# before the send - the message-losing direction HelloWorldSungin/firstmate#84
# calls out, since an approval dialog owning the keyboard can consume our Enter
# while the same pre-existing blocked turn keeps reporting itself. On an idle
# native baseline, a rendered busy footer may supply the same generating signal
# because live Claude never leaves idle. The policy is
# fm_composer_queued_enter_verdict; this adapter only supplies the busy
# primitive.
# Echoes empty|pending|unknown|send-failed|unverifiable, a subset of the
# proof-carrying submit vocabulary. Empty means confirmed submitted for every
# backend; how each backend confirms it is an internal decision.
#
...
fm_backend_herdr_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle> [expected-label] [expected-harness]
  ...
  local footer_baseline='' allow_rendered=0
  if [ "$#" -ge 7 ]; then
    declared_harness=$7
    probe_identity=1
  fi
...
EOF
  ...
  case "$declared_harness" in
    agy) printf 'send-failed'; return 0 ;;
  esac
  ...
  # and before the first Enter is still a pre-submission baseline. The
  # rendered-footer conversion below refuses a native `working` baseline
  # outright, so skip that read entirely in that case rather than spending a
  # pane read no verdict can use.
  if [ "$baseline" = idle ]; then
    allow_rendered=1
  elif [ "$baseline_raw" != working ]; then
  ...
  while :; do
    ...
    if [ "$baseline" = idle ]; then
      ...
      sleep "$sleep_s"
      verdict=$(fm_backend_herdr_composer_state "$target")
      if [ "$verdict" = pending ] && [ "$baseline_raw" != working ] \
