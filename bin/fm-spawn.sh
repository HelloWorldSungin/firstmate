...
#   Ship/design/scout launches always supply fm-dod-lib.sh's current worker role scope
...
#   overrides it for this spawn (subject to its role support); agy is additionally
...
#   The raw path refuses detectable agy spellings at spawn
#   and installs PATH guard shims as execution-time defense; use --harness for
#   the restricted adapter so the required gates, trust setup, and supervision apply.
...
#   bin/fm-design-skills.sh resolve that gates a fresh dispatch, records
...
#   cleanup even if the captain-owned install later changes. A --relaunch of
#   that same design task reuses the recorded release and the dispatch-pinned
#   skill paths rather than resolving again
...
#   That placement is proven only at launch. Every ship, design, or scout pane therefore
...
#   not marked.
#   Only after this isolation check, every fresh ship, design, or scout requires a clean
...
#   opts every launch (ship, design, scout, secondmate, raw command, and relaunch) into
...
#   marker FM_TASK_ID that ship, design, and scout panes receive above.
...
# bin/fm-launch-lib.sh owns the settings construction and its rationale.
# Publishing the record and moving this home's backlog item to In flight are one
# step, not two: bin/fm-backlog-transition-lib.sh owns that invariant, and this
# script performs the transition under the task's own meta lock before it reports
# success. A ship, design, or scout dispatch therefore REFUSES up front, before any
...
case "$HARNESS" in
  pi|pi-signed)
    ...
    # __PIBIN__/__PITUIMODE__ are substituted after fm_launch_render, which
    # passes them through untouched, so the resolved executable path never sits
...
case "$LAUNCH" in
  *__MUSEBIN__*)
    ...
    LAUNCH=${LAUNCH//__MUSEBIN__/$(fm_launch_shell_quote "$MUSE_BIN")}
    LAUNCH=${LAUNCH//__MUSECONFIG__/$(fm_launch_shell_quote "$MUSE_CONFIG_HOME")}
    LAUNCH=${LAUNCH//__MUSEDATA__/$(fm_launch_shell_quote "$MUSE_DATA_HOME")}
    ;;
esac
...
if [ "$KIND" != secondmate ]; then
  ...
  case "$HARNESS" in
    claude*)
      ...
      mkdir -p "$WT/.claude"
      busy_cmd_prefix="$(fm_launch_shell_quote "$FM_ROOT/bin/fm-busy-event.sh") apply $(fm_launch_shell_quote "$STATE_REAL") $(fm_launch_shell_quote "$ID")"
      busy_suffix="--gen $(fm_launch_shell_quote "$BUSY_GEN") --source claude-hook"
...
EOF
    ...
    gemini)
      ...
      busy_cmd_prefix="$(fm_launch_shell_quote "$FM_ROOT/bin/fm-busy-event.sh") apply $(fm_launch_shell_quote "$STATE_REAL") $(fm_launch_shell_quote "$ID")"
      busy_suffix="--gen $(fm_launch_shell_quote "$BUSY_GEN") --source gemini-hook"
      g_before=$(json_escape "$busy_cmd_prefix busy $busy_suffix --event before-agent >/dev/null 2>&1 || true; printf '{}'")
      g_after=$(json_escape "touch $(fm_launch_shell_quote "$TURNEND"); $busy_cmd_prefix idle $busy_suffix --event after-agent >/dev/null 2>&1 || true; printf '{}'")
...
if [ "$HARNESS" = claude ]; then
  ...
  # The endpoint and worktree already exist and this record is the only thing
  # naming them, so a watermark failure keeps the record instead of unwinding
  # the publication: rolling it back here would strand a live pane and local
  # copy with nothing pointing at them. The backlog item stays Queued, which
  # bootstrap's worker-record reconciliation converges to In flight on the next
...
EFFORTFLAG=$(fm_launch_effort_flag "$HARNESS" "$EFFORT" "$MODEL") || exit 1
ROVOCONFIGOVERRIDE=
if [ "$HARNESS" = rovo ]; then
  ROVOCONFIGOVERRIDE=$(fm_launch_rovo_config_override_flag "$EFFORT" "$DATA" "$STATE" "$ID") || {
...
LAUNCH=$(fm_launch_render \
  ...
  __OMPEXT__ "$(fm_launch_shell_quote "$STATE/$ID.omp-ext.ts")" \
  __OMPWORKERCFG__ "$(fm_launch_shell_quote "${OMP_WORKER_CFG:-$FM_ROOT/.omp/fm-worker-overlay.yml}")" \
  __GEMINISETTINGS__ "$(fm_launch_shell_quote "$STATE_REAL/$ID.gemini-settings.json")" \
  __ROVOBIN__ "$(fm_launch_shell_quote "${ROVO_BIN:-}")" \
  __ROVOCONFIGOVERRIDE__ "$ROVOCONFIGOVERRIDE") || {
...
# suite in the repository's primary checkout. Ship, design, and scout workers are the
...
# syntax of its own.
if [ "$KIND" = ship ] || [ "$KIND" = design ] || [ "$KIND" = scout ]; then
...
if [ "$LAUNCH_ENV_ENABLED" = 1 ]; then
  ...
  LAUNCH="$LAUNCH_ENV_PREFIX /bin/sh -c $(fm_launch_shell_quote "$LAUNCH")"
fi
sleep 0.3
