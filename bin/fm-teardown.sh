...
# publish the durable outcome manifest, clear volatile state, transition this home's
# backlog item for ship, design, and scout tasks before reporting success (a
# secondmate teardown closes none, since secondmates are not backlog items), then
# refresh the project's clone for PR-based ship or design tasks and print a
# backlog-refresh reminder for ship, design, and scout teardowns.
# Removing state/<id>.meta and landing the backlog transition are one step, not two:
...
# state/<id>.backlog-close before any destructive step, so a process killed
# anywhere in that sequence leaves the next session start enough to finish it;
# a landed transition removes that record.
# A deliberate refusal is not a crash, so every refusal that DELIBERATELY keeps
# this task - an unpublishable completion manifest, a model-routing verdict that
# gained a turn, a Herdr close that could not be confirmed, or an undelivered parent outcome - withdraws that
# pending close through withdraw_pending_backlog_close before it exits. Without
# that withdrawal the next session start would replay the close and remove the
# very record those refusals just preserved, which is the opposite of what each
# of them is for.
...
# secondmate's home, which can contain those records, and before the backlog
# close that removes the record the manifest is composed from - so the task stays
# in durable history afterwards. A manifest that cannot be published refuses the
...
remote_secondmate_teardown() {
  ...
  rm -f -- "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.progress" \
...
# A refusal that deliberately keeps this task must also withdraw the close it
# recorded on the way in. The pending-close record exists so a CRASH mid-cleanup
# can still be finished; replaying it after a refusal that preserved the task on
# purpose would remove the record that refusal exists to keep. Best effort and
# loud: a marker that cannot be withdrawn is named so the operator can remove it
# before the next session start replays it.
withdraw_pending_backlog_close() {
  [ "${BACKLOG_CLOSED:-0}" = 1 ] || return 0
  local marker
  BACKLOG_CLOSED=0
  marker=$(fm_backlog_close_marker_path "$STATE" "$ID") || return 0
  [ -e "$marker" ] || [ -L "$marker" ] || return 0
  fm_backlog_close_marker_remove "$marker" "$STATE" || {
    echo "warning: task $ID is preserved but its pending backlog close could not be withdrawn ($FM_BACKLOG_TRANSITION_ERROR); remove $marker by hand before the next session start replays it" >&2
    return 1
  }
  return 0
}
...
# to attribute is one it would otherwise have aborted. Branch identity is the
# task's recorded branch: a pooled worktree reallocated to a live second task
...
cleanup_firstmate_home_children() {
  local home=$1 sub_state child_meta child_id child_t child_wt child_proj child_kind child_home child_backend child_orca_worktree_id child_return_rc child_busy_gen child_nativeturnend_key child_model_output child_owner_rc
  sub_state="$home/state"
  [ -d "$sub_state" ] || return 0
  for child_meta in "$sub_state"/*.meta; do
    [ -e "$child_meta" ] || continue
    ...
    rm -f "$sub_state/$child_id.turn-ended" "$sub_state/$child_id.progress" \
      ...
      "$sub_state/$child_id.usage-sessions" "$sub_state/$child_id.muse-session" \
      "$sub_state/$child_id.muse-session-current" \
...
if [ "$KIND" = design ] && [ "$FORCE" != "--force" ]; then
  if ! FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$STATE" FM_DATA_OVERRIDE="$DATA" \
      FM_CONFIG_OVERRIDE="$CONFIG" "$SCRIPT_DIR/fm-captain-hold.sh" verify "$ID" >/dev/null; then
    echo "REFUSED: design task $ID has not passed the captain-call completion gate." >&2
    echo "Inventory its ADR and design interview through bin/fm-captain-hold.sh before teardown." >&2
    exit 1
  fi
fi
...
if [ "$NO_VERDICT_CLEANUP_CANDIDATE" -eq 1 ]; then
  ...
  if ! model_verdict_precedes_any_turn "$MODEL_VERDICT"; then
    ...
    withdraw_pending_backlog_close || true
    exit 1
  fi
...
if [ "$NO_VERDICT_RETAIN_WORKTREE" -eq 1 ] || ! teardown_owns_worktree; then
  :
...
# on the next line.
publish_outcome_manifest "$FORCE" || { withdraw_pending_backlog_close || true; exit 1; }
if [ "$KIND" = secondmate ]; then
  [ -n "$HOME_PATH" ] || HOME_PATH=$WT
  handoff_wake_retire_stage \
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.progress" \
  "$STATE/$ID.pi-ext.ts" "$STATE/$ID.omp-ext.ts" "$STATE/$ID.run-step" \
