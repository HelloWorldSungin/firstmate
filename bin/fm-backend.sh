...
# fm_backend_target_presence_state: safety-grade endpoint presence proof for
# callers that must distinguish authoritative absence from a failed read.
# Prints exactly missing, ambiguous, or unreadable. `missing` requires a
# successful authoritative inventory that excludes the recorded endpoint;
# any live container or unresolved identity is `ambiguous`, and any command or
# parse failure is `unreadable`.
fm_backend_target_presence_state() {  # <backend> <target> [expected-label]
  local backend=$1 target=$2 expected_label=${3:-}
  local state sessions panes tabs session pane scoped count out inventory workspace
  case "$backend" in
    tmux|herdr)
      state=$(fm_backend_agent_state "$backend" "$target")
      case "$state" in
        missing) printf 'missing' ;;
        unreadable|unverified) printf 'unreadable' ;;
        *) printf 'ambiguous' ;;
      esac
      ;;
    zellij)
      ...
      printf '%s' "$panes" | jq -e 'type == "array"' >/dev/null 2>&1 \
        || { printf 'unreadable'; return 0; }
      count=$(printf '%s' "$panes" | jq -r --argjson pane "$pane" \
        '[.[]? | select(.id == $pane and .is_plugin == false)] | length' 2>/dev/null) \
        || { printf 'unreadable'; return 0; }
      if [ "$count" != 0 ]; then
        printf 'ambiguous'
        return 0
      ...
      count=$(printf '%s' "$tabs" | jq -r --arg scoped "$scoped" --arg legacy "$expected_label" \
        '[.[]? | select(.name == $scoped or .name == $legacy)] | length' 2>/dev/null) \
        || { printf 'unreadable'; return 0; }
      if [ "$count" = 0 ]; then
        printf 'missing'
      else
      ...
      ;;
    orca)
      fm_backend_source orca || { printf 'unreadable'; return 0; }
      out=$(orca terminal read --terminal "$target" --limit 1 --json 2>/dev/null) \
        || { printf 'unreadable'; return 0; }
      printf '%s' "$out" | fm_backend_orca_json_text >/dev/null 2>&1 \
        || { printf 'unreadable'; return 0; }
      printf 'ambiguous'
      ;;
    cmux)
      ...
      inventory=$(fm_backend_cmux_cli workspace list --json --id-format uuids 2>/dev/null) \
        || { printf 'unreadable'; return 0; }
      printf '%s' "$inventory" | jq -e '.workspaces | type == "array"' >/dev/null 2>&1 \
        || { printf 'unreadable'; return 0; }
      if [ -n "$expected_label" ]; then
        scoped=$(fm_backend_cmux_scoped_title "$expected_label")
        count=$(printf '%s' "$inventory" | jq -r --arg id "$workspace" --arg title "$scoped" \
          '[.workspaces[]? | select(.id == $id or .title == $title)] | length' 2>/dev/null) \
          || { printf 'unreadable'; return 0; }
      else
        count=$(printf '%s' "$inventory" | jq -r --arg id "$workspace" \
