...
# bootstrap running its detect-only diagnostics without its ten mutating
...
#                       (legacy PR-check and endpoint-binding migrations,
#                       proven run attribution, secondmate convergence and
#                       liveness, pending handoff retry, Relay artifact writes,
#                       fleet sync, project-board reconciliation, and
#                       token-usage refresh) run only when locked; network
...
#                       durable queue and only run when locked.
...
# check, fork drift checks, GBrain serving-credential state, legacy run-
# attribution blindness, the harness override, crew-dispatch validation,
...
# Only projection cleanup, bootstrap mutating sweeps, inactive-outcome
...
for meta in "$STATE"/*.meta; do
  ...
  if [ -n "$window" ]; then
    ...
    if fm_backend_target_exists "$backend" "${target:-$window}" "fm-$id" "$(fm_meta_get "$meta" zellij_tab_id)"; then
