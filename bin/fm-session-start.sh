...
if [ -f "$STATE/.afk-contract" ]; then
  ...
  if [ "$AFK_MODE" = quiet ]; then
    printf 'present - quiet-mode supervision is active; the daemon owns the watcher, only an explicit /quiet off exits it (attended mode without an away record).\n'
  else
