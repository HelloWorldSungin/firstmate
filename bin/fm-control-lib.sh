...
# than guessed at, exactly as a spawn on it would be. agy is the fork-local
# crew-only adapter (AGENTS.md section 4); its rows below were measured on
# agy 1.1.14 and are recorded in docs/verification/runtime-backends.md.
...
# follow-up` placeholder, so it needs no clear key. agy was checked the same way
# and also does not repollute: a single Escape closes the turn with
# `Interrupted - What should Antigravity CLI do instead?` and leaves a bare `>`
# composer. Prints the key or nothing; a harness with no verified mechanics
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    # agy prints an `Interrupted` line but exposes no durable typed close: its
    # busy state comes from herdr's native agent-state only, so there is no
    # source an acknowledgement claim could be built on.
