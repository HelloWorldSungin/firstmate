...
# The shared library is the sole settings mutation owner, including locking,
# atomic replacement, external-write detection, and created/preexisting identity.
# This command owns the upstream linked-worktree scope check above.
# shellcheck source=bin/fm-agy-trust-lib.sh
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/fm-agy-trust-lib.sh"
export FM_AGY_SETTINGS_OVERRIDE="$STORE"
fm_agy_trust_add "$WT_LOGICAL" || refuse "could not record trust for '$WT_LOGICAL' in '$STORE'"
if [ "$WT_LOGICAL" != "$WT_REAL" ]; then
  fm_agy_trust_add "$WT_REAL" || refuse "could not record trust for '$WT_REAL' in '$STORE'"
fi
