...
#   4. No run for this crew (pre-validation, or a kind that never validates its
#      own worktree - scout and secondmate): fall back to the
...
# A design task does drive one: under no-mistakes its ADR ships through the same
# pipeline on its own branch, so its run step must stay authoritative here rather
# than degrading to the pane/log fallback.
if { [ "$KIND" = ship ] || [ "$KIND" = design ]; } && [ -n "$CREW_BRANCH" ] && command -v no-mistakes >/dev/null 2>&1; then
