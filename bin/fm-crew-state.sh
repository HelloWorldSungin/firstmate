...
# every heartbeat:
#
#   state: <working|parked|done|blocked|paused|failed|unknown|abandoned> · source: <run-step|run-step-degraded|run-attribution|completion-attestation|pane|status-log|remote-endpoint|none> · <detail>
#
# Logic, in order:
...
#      gone/dead. For a local ship or design task, branch= in that task record
#      is the authoritative task branch: fm-spawn copies it from fm-brief's
#      exact firstmate-task-branch marker, while fm-promote writes it alongside
#      the promotion's exact branch instruction. The worktree's ambient branch
#      is only observed evidence, never task identity. A named ambient branch
#      that differs from the recorded branch is an attribution fault, not
#      permission to inspect that branch's run or fall back to the status log.
...
#      A terminal failed CI monitor may read held-for-merge done only under
#      nm_failed_run_is_green_held_ci; a failed coarse ledger with an unavailable
#      daemon reads unknown under nm_daemon_probe_down.
#   3. Reconcile the status log: if its last line says needs-decision/blocked but
#      the run-step shows the run moved on, the log is deterministically stale and
#      is flagged superseded. A genuinely parked run plus a needs-decision log
#      agree, and are reported as parked. Positive daemon socket failure stays
#      blocked; other transport claims can be superseded by recent run activity.
...
#      than trusting a stale status log. A tmux/Herdr capture failure consults
#      the recovery classifier: only positive death means gone; an unanswered
#      probe is unreachable, and an alive endpoint retains normal classification.
...
# a step is actually running or fixing. The header's indentation bounds the
# block; callers consume the canonical step/status column prefix shown above.
nm_steps_rows() {
  printf '%s\n' "$RUN_OUT" | awk '
...
lookup_coarse_run() {
  ...
  case "$COARSE_EVIDENCE" in
    attributable)
      ...
      ;;
    inconclusive)
      if [ "$COARSE_STATUS" = malformed ]; then
        LOOKUP_DEGRADED_REASON="runs ledger malformed"
      else
