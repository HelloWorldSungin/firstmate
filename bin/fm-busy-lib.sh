...
#   5. an untrusted agy record on Herdr may classify busy only when one native
#      sample reports both the matching identity and working status
...
fm_busy_classify() {  # <backend> <target> <harness> <id> <state-dir> [tail40]
  ...
  local out rc r_state r_source native='' native_identity='' native_sample='' native_identity_required=0 log
  case "$harness" in
    kimi*)
  ...
  if [ "$rc" = 0 ]; then
    ...
    case "$backend:$harness" in
      herdr:agy) native_identity_required=1 ;;
  ...
  if [ "$backend:$harness" = herdr:agy ]; then
    printf 'unknown herdr-native'
    return 0
  fi
  case "$harness" in
    muse*)
...
fm_busy_classify_live() {  # <backend> <target> <harness> <id> <state-dir> [expected-label] [expected-tab-id]
  local backend=$1 target=$2 harness=$3 id=$4 state=$5 label=${6-} tab_id=${7-}
  if [ -z "$target" ]; then
    printf 'unknown no-target'
    return 0
  fi
  if ! fm_backend_target_exists "$backend" "$target" "$label" "$tab_id" 2>/dev/null; then
