...
# reaches the same verdicts from its own native busy state and structural
# composer read rather than reusing these tmux internals, so neither adapter
# papers over the other's shape; its boundary is owned by
