...
# The parked-decision marker is reconciled on the same event, because that
# marker keys on the task's endpoint rather than on the status log's position
# and must follow whether the task still holds an open decision.
