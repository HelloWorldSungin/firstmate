...
printf '%s\n' "$SNAPSHOT" | jq -r '
  ...
  def model_suspect($t): $t.model_verification.verdict == "mismatch" or $t.model_verification.verdict == "unverifiable" or $t.model_verification.verdict == "unstarted" or $t.model_verification.verdict == "unarmed";
  def model_row($t):
