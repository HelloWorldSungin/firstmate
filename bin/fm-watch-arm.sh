...
# The arm pid that owned <watcher-pid>'s cycle, read from this home's own cycle
# ledger, or nothing when no row records it.
predecessor_arm_pid_of_watcher() {  # <watcher-pid>
  local watcher=$1
  [ -f "$CYCLE_LOG" ] || return 0
  awk -F'\t' -v target="watcher_pid=$watcher" '
    $2 == target { split($1, field, "="); found = field[2] }
...
if [ "$mode" = restart ]; then
  ...
  if fm_pid_alive "$lock_pid"; then
    if fm_watcher_lock_matches_pid "$STATE" "$WATCH" "$lock_pid" "$FM_HOME"; then
      ...
      # A restart deliberately ends this home's own cycle and starts the next
      # one, which is exactly the predecessor/successor relationship the arm
      # layer already models. Declaring it here routes the replacement through
      # that path, so it takes the lock and stays up instead of opening with a
      # `check: rearm-resurface` and exiting - which would leave the restart
      # with no live watcher at all, the defect this home's restart-handover
      # guarantee exists to prevent. Prefer the predecessor's true arm pid from
      # the cycle ledger so its final record still links to this successor; with
      # no such row there is nothing to link, and the stopped watcher's own pid
      # carries only the successor marking, which is the half that matters here.
      FM_WATCH_PREDECESSOR_ARM_PID=$(predecessor_arm_pid_of_watcher "$lock_pid")
      [ -n "$FM_WATCH_PREDECESSOR_ARM_PID" ] || FM_WATCH_PREDECESSOR_ARM_PID=$lock_pid
    else
