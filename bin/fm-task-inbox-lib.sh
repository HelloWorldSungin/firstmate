...
# adapter has its own submit primitive: on herdr, agy is delivered
# through Herdr's atomic `agent prompt`, selected by this value alone, so
# omitting it silently downgrades its doorbell to the typed composer path this
# fleet does not trust for agy. Every ring caller resolves it from the target's
# own metadata.
fm_task_inbox_ring() {  # <backend> <target> <record-path> [expected-label] [expected-harness]
  ...
  # Accepted residual race: on the typed transport, input and Enter are separate delivery
  # steps, so an agent exiting after the liveness check could leave a bare
  # shell only a suffix; the `: ` prefix protects complete lines only. The harness-specific
  # atomic submit path remains selected by the recorded harness below.
