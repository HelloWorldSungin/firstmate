...
worker_stop_active_execution() {
  ...
  # Reap the whole supervised tree BEFORE the lane loop kills its children. A
  # lane's serving child spawns its own commands, and once that child dies its
  # orphans are re-parented away from this supervisor, so the descendant walk
  # can no longer reach them. Signalling their process groups while the tree is
  # still intact is what stops them polling after the supervisor is gone. A
  # failed reap is folded into the verdict rather than returned early, so each
  # lane still releases its recorded claim.
  worker_reap_descendants || failed=1
  while [ "$i" -lt "$count" ]; do
...
worker_supervise_linux() {
  local account_home child_pid child_status started failures=0 restarts=0 backoff
