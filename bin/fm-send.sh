...
# delivered.
# A cursor/agy atomic prompt that is known undelivered exits 1; one whose
# acceptance cannot be verified reports verdict=unverifiable and also exits 3,
# so callers treat it as neither success nor safe-to-resend failure.
...
if [ "${1:-}" = "--key" ]; then
  ...
  case "$verdict" in
    ...
    send-failed)
      ...
      ;;
    unverifiable)
      # Herdr's atomic agy prompt could not prove acceptance either way, so the
      # request's delivery state stays explicitly unknown rather than discarded.
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
        fm_pending_reply_mark_delivery_unknown "$STATE" "$PENDING_REPLY_CORR" || true
      fi
      echo "error: text delivery unverifiable on $T (verdict=unverifiable; tried $RESOLUTION_TRIED)" >&2
      exit 3
      ;;
    pending)
