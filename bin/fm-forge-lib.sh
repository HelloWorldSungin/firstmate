...
# FM_FORGE_HTTP_CODE and FM_FORGE_REASON are this library's published outputs,
# assigned throughout and read only by sourcing scripts shellcheck cannot see.
...
#
# WRITE ALLOWLIST. The fm_gitea_* functions below are the only write entry
# points, and they cover exactly what firstmate's tracker lifecycle needs:
# reading an issue and its comments, creating or editing firstmate's own status
# comment, posting the merge-link comment, and closing the issue. There is no
# general request function exported, and nothing here can touch a branch,
# release, repository setting, permission, or any other forge object - a wider
# credential on disk gains no extra reach through this library, which is what
# lets the captain issue a narrow token and revoke a wide one.
#
...
# and FM_FORGE_HTTP_CODE holding the code when one was received. Callers stay
# fail-open: they warn with the reason and exit 0, never fail the operation
# that already succeeded around them.
#
...
# owner, which callers must report rather than work around.
fm_forge_token_read() {  # <config-dir> <host>
  local config=$1 host=$2 path mode
...
fm_forge_curl() {  # <bound> <token> <curl-args...>
...
_fm_forge_gitea_call() {  # <bound> <token> <host> <method> <api-path> <out-file> [json-body-file]
  ...
  code=$(fm_forge_curl "$bound" "$token" "${args[@]}" "https://$host$api" 2>/dev/null) || rc=$?
  case "$rc" in
    0) ;;
    28|124|137)
...
fm_gitea_comment_update() {  # <bound> <token> <host> <path> <comment-id> <body-file>
  local payload rc=0
  payload=$(mktemp "${TMPDIR:-/tmp}/fm-forge.XXXXXX") || {
