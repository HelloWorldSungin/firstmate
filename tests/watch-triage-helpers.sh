#!/usr/bin/env bash
# tests/watch-triage-helpers.sh - the always-on wake triage built into
...
test_explicit_decision_view_preserves_the_durable_fold() {
  local dir file explicit retained
  dir=$(make_case explicit-decision-view); file="$dir/state/view.status"
  printf '%s\n' 'blocked: unkeyed dependency' 'needs-decision: [key=choice] explicit choice' > "$file"
  retained=$(status_open_decisions "$file")
  explicit=$(status_open_decisions "$file" --explicit-only)
  assert_contains "$retained" $'default\tblocked\t' "unkeyed blocker must remain durably open"
  assert_contains "$explicit" $'choice\tneeds-decision\t' "note-head explicit key must remain eligible"
  assert_not_contains "$explicit" $'default\t' "implicit default must not suppress repaint alarms"
  printf '%s\n' 'blocked [key=default]: explicitly named default' >> "$file"
  explicit=$(status_open_decisions "$file" --explicit-only)
  assert_contains "$explicit" $'default\tblocked\t' "literal default key is explicitly keyed"
  printf '%s\n' 'blocked: replacement without a key' >> "$file"
  explicit=$(status_open_decisions "$file" --explicit-only)
  assert_not_contains "$explicit" $'default\t' "unkeyed replacement must lose explicit provenance"
  assert_contains "$(status_open_decisions "$file")" 'replacement without a key' "filter must not close a durable blocker"
  printf '%s\n' 'resolved: [key=choice] choice handled' >> "$file"
  [ -z "$(status_open_decisions "$file" --explicit-only)" ] || fail "resolved explicit key remained eligible"
  pass "explicit-key view preserves durable unkeyed retention and default-key provenance"
}
...
test_parked_decision_survives_pane_repaint() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture" \
    ...
    FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "watcher did not surface a newly parked captain decision"
  ...
  assert_contains "$(status_open_decisions "$state/parked.status")" $'review-gate\t' \
    "acknowledging a wake must not close the keyed decision"
...
test_resolved_decision_can_reopen_identically() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture" \
    ...
    FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "watcher did not surface the original keyed decision"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture" \
    ...
    FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "the decision resolution did not surface"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture" \
    ...
    FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "an identical decision did not surface after resolving and reopening"
...
test_new_keyed_decision_on_parked_pane_surfaces() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture" \
    ...
    FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "watcher did not surface the first parked decision"
  FM_STATE_OVERRIDE="$state" "$DRAIN" > /dev/null 2>&1 || true
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture" \
    ...
    FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "a NEW keyed decision on a parked pane did not surface"
...
test_parked_decision_with_dead_agent_wedge_escalates() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture" \
    ...
    FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "watcher did not surface the parked decision before the wedge case"
  FM_STATE_OVERRIDE="$state" "$DRAIN" > /dev/null 2>&1 || true
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "a parked crew whose agent died was not detected as a wedge suspect"
...
test_paused_resurface_backs_off_while_wedge_still_escalates() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "the first recheck of a long declared wait did not re-surface"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "a declared wait past its widened window did not re-surface"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "an undeclared idle crew past the wedge threshold did not escalate"
...
test_exited_declared_pause_is_bounded_but_live_captain_held_surfaces() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "live captain-held pane did not surface immediately"
...
test_live_declared_pause_is_absorbed_on_the_designed_cadence() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "a live crew's declared pause never reached the designed recheck"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "a live crew that declared no wait stopped escalating past the wedge threshold"
  grep -F "possible wedge" "$out" >/dev/null \
...
SH
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "the pane that declared its wait mid-classification never surfaced at all"
  bare=$(awk -F '\t' -v w="$window" '$3 == "stale" && $4 == w && $5 == "stale: " w { n++ } END { print n + 0 }' "$state/.wake-queue")
  [ "$bare" -eq 0 ] \
...
test_live_declared_pause_still_recoverable_once_its_agent_dies() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "the live parked pane never reached its declared-wait recheck"
  [ "$(pause_streak_count "$state/.paused-streak-$key")" = 1 ] \
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "a parked pane whose agent died went silent instead of re-surfacing"
  grep -F "awaiting external" "$out" >/dev/null \
...
test_absorbed_replacement_wait_does_not_inherit_the_old_throttle() {
  ...
  do
    ...
    printf '%s\n' "$replacement" >> "$statusf"
    # The fork rechecks a replacement at its own base deadline, rather than
    # immediately. It must not inherit the prior wait's widened window.
    set_mtime "$(( $(date +%s) - 300 ))" "$statusf"
    sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-held_status"
...
# A live captain-held pane earns one initial inspection and then a
# declaration-scoped throttle. Declared paused workers use the fork's widening
# cadence instead, covered by the live-declared-pause cases above.
test_live_declared_wait_churn_honors_the_resurface_throttle() {
  ...
  replacement='captain-held [key=release]: awaiting the captain on the release call'
  printf '%s\n' "$replacement" >> "$statusf"
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-parked_status"
  ...
  pass "a live captain-held worker surfaces once, absorbs pane churn for the whole re-surface window, then re-surfaces when it elapses"
}
...
test_stale_churn_without_a_captain_call_still_alarms() {
  ...
  do
    ...
    while [ "$round" -le 2 ]; do
      ...
      if [ "$name" = unheld-blocker ]; then
        assert_contains "$(status_open_decisions "$state/held-merge.status")" $'default\tblocked\t' \
          "unkeyed blocker must remain open after each acknowledged repaint wake"
      fi
      round=$((round + 1))
    done
  done
...
test_stranded_run_still_wedge_escalates() {
  ...
  pid=$!
  wait_for_exit "$pid" 100 || { unset FM_FAKE_TMUX_CURRENT_COMMAND; fail "a stranded validation run did not wedge-escalate"; }
...
test_wedged_crew_with_no_run_escalates_unchanged() {
  ...
  pid=$!
  wait_for_exit "$pid" 100 || { unset FM_FAKE_TMUX_CURRENT_COMMAND; fail "a wedged crew with no active run did not escalate"; }
...
test_dead_agent_escalates_even_while_its_run_progresses() {
  ...
  pid=$!
  wait_for_exit "$pid" 100 || { unset FM_FAKE_TMUX_CURRENT_COMMAND; fail "a dead agent was absorbed because its run was progressing"; }
...
test_progressing_run_escalates_anyway_past_the_hold_cap() {
  ...
  run_wedge_watcher "$state" "$fakebin" "$window" "$capture" "$out" "$verdict" FM_RUN_PROGRESS_HOLD_MAX=2
  pid=$!
  wait_for_exit "$pid" 100 || { unset FM_FAKE_TMUX_CURRENT_COMMAND; fail "a pane held to the cap never escalated: $(cat "$out")"; }
  grep -F "possible wedge" "$out" >/dev/null \
...
test_busy_pane_progressing_run_holds_then_escalates_past_the_cap() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || { unset FM_FAKE_TMUX_CURRENT_COMMAND; fail "a busy pane held to the cap never escalated: $(cat "$out")"; }
  grep -F "possible wedge" "$out" >/dev/null \
