...
# Every thread window this suite seeds is computed from the suite's own clock.
# A hardcoded calendar timestamp is a dated bomb: the day it elapses, every seeded
# commitment reads as an unreachable thread and the whole suite turns red on every
# branch. Deriving the windows here keeps the assertions identical and removes the
# calendar deadline from them.
SUITE_NOW=$(date -u +%s)
# GNU date reads an epoch with `-d @<n>`, BSD date with `-r <n>`. Probe once with a
# known epoch rather than falling back on error: on GNU, `-r` reads a FILE, so an
# error-driven fallback could silently format some file's mtime instead.
if [ "$(date -u -d @0 +%Y 2>/dev/null)" = 1970 ]; then
  iso_utc() { date -u -d "@$1" +%Y-%m-%dT%H:%M:%SZ; }  # <epoch-seconds>
else
  iso_utc() { date -u -r "$1" +%Y-%m-%dT%H:%M:%SZ; }  # <epoch-seconds>
fi
# Intake is always behind now; a seeded thread stays open well past the 48-hour
# closing window so only a test that overrides the clock sees a deadline.
SUITE_RECEIVED_AT=$(iso_utc $((SUITE_NOW - 7 * 86400)))
THREAD_EXPIRES_EPOCH=$((SUITE_NOW + 7 * 86400))
THREAD_EXPIRES_AT=$(iso_utc "$THREAD_EXPIRES_EPOCH")
# The backlog obligation's own deadline, deliberately outside the thread window.
OBLIGATION_EXPIRES_AT=$(iso_utc $((SUITE_NOW + 30 * 86400)))
...
seed_commitment() {
  ...
  jq -n --arg r "$request" --arg p "$platform" \
    --arg received "$SUITE_RECEIVED_AT" --arg expires "$THREAD_EXPIRES_AT" \
...
test_rechain_refuses_unclaimed_existing_destination() {
  ...
  tasks_in "$home" public-followup add public-final-existing-b \
    ...
    --expires-at "$OBLIGATION_EXPIRES_AT" >/dev/null || fail "could not seed destination collision"
