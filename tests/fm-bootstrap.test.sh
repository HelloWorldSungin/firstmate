...
# chrome-devtools-axi version is on PATH, and which lavish-axi version is on PATH.
...
make_fake_toolchain() {
  ...
  fm_fake_version_tool "$fakebin" lavish-axi FM_FAKE_LAVISH_AXI_VERSION 0.1.62
  cat > "$fakebin/gh-axi" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${FM_FAKE_GH_AXI_VERSION:-0.1.34}"
  exit 0
fi
exit 0
SH
...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${FM_FAKE_QUOTA_AXI_VERSION:-0.1.32}"
  exit 0
fi
exit 0
SH
...
minimum gh-axi version is accepted^0.1.34^empty
newer gh-axi patch is accepted^0.1.35^empty
newer gh-axi minor is accepted^0.2.0^empty
newer gh-axi major is accepted^1.0.0^empty
older gh-axi patch reports an upgrade^0.1.33^missing
...
test_chrome_devtools_axi_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    [ -n "$label" ] || continue
    n=$((n + 1))
    case_dir="$TMP_ROOT/chrome-devtools-axi-$n"
    mkdir -p "$case_dir/home/config"
    printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
...
minimum chrome-devtools-axi version is accepted^0.1.30^empty
newer chrome-devtools-axi patch is accepted^0.1.31^empty
newer chrome-devtools-axi minor is accepted^0.2.0^empty
newer chrome-devtools-axi major is accepted^1.0.0^empty
the patch just below the floor reports an upgrade^0.1.29^missing
...
minimum quota-axi version is accepted^0.1.32^empty
newer quota-axi patch is accepted^0.1.33^empty
newer quota-axi minor is accepted^0.2.0^empty
newer quota-axi major is accepted^1.0.0^empty
the patch just below the floor reports an upgrade^0.1.31^missing
...
gemini default is accepted^{"default":{"harness":"gemini"}}^empty^
gemini rule records omitted effort^{"rules":[{"when":"Gemini worker","use":{"harness":"gemini","effort":"high"}}]}^empty^
gemini array profile is accepted^{"default":[{"harness":"gemini"},{"harness":"codex"}]}^empty^
