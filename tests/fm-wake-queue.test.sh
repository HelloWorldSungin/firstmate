...
seed_stall_mate() {  # <case-name> <age-seconds> <busy-state|none>
  local name=$1 age=$2 busy=$3 dir state sub real_date now epoch
  dir=$(make_case "$name")
  state="$dir/state"
  sub="$dir/secondmate"
...
SH
  ...
  # Seed the observed progress through its owner rather than timing out a
  # setup watcher: interruption legitimately leaves a recovery obligation that
  # must surface before the later checkpoint can be quiet. Separate episode
  # tests exercise the first observation, and recovery tests retain that wake.
  real_date=$(command -v date)
  now=$(date +%s)
  FM_STATE_OVERRIDE="$state" bash -c '
    # shellcheck disable=SC1090,SC1091
    . "$1"
    fm_wake_secondmate_progress_marker_write mate "$2" "$3"
  ' _ "$ROOT/bin/fm-wake-lib.sh" "$now" "$epoch-7" \
    || fail "could not seed the observed queue position"
...
test_busy_secondmate_does_not_alarm_on_an_aged_row() {
  ...
  expect_code 124 "$status" "busy secondmate quiet checkpoint: $(cat "$dir/watch.out") $(cat "$dir/watch.err")"
...
# A busy record cannot suppress a genuinely frozen queue indefinitely.
# The controlled clock ages both the observation and the active-turn boundary,
# while leaving the semantic verdict busy so this cannot pass as an idle case.
test_secondmate_busy_suppression_is_bounded() {
  local dir status=0
  dir=$(seed_stall_mate secondmate-stall-busy-bound 4000 busy)
  run_stall_checkpoint "$dir" || status=$?
  expect_code 0 "$status" "over-age busy turn must permit a stall inspection"
  grep -F 'state=busy' "$dir/watch.out" >/dev/null \
    || fail "bounded suppression did not retain the established busy verdict"
  [ -s "$dir/state/.wake-queue" ] || fail "bounded busy suppression lost the durable notification"
  pass "busy suppression expires without treating a busy verdict as idle"
}
...
test_live_presentation_holder_is_deadlined_without_weakening_ack() {
  ...
  # A pending queue still requires supervision in the fork, so its guard
  # advisory is expected even while presentation waits for its cursor lock.
  grep -Fx 'WARNING: queued wakes pending - drain them with bin/fm-wake-drain.sh before anything else.' "$first_err" >/dev/null \
    || { kill "$presentation_holder" 2>/dev/null || true; fail "presentation contention lost the pending-queue advisory"; }
  if grep -Ev '^WAKE_ACK_REQUIRED:|^●|^WARNING: queued wakes pending - drain them with bin/fm-wake-drain\.sh before anything else\.$|^This is a supervision warning only; the guarded operation WILL still run\.$' "$first_err" | grep . >/dev/null; then
