...
# --- a refusal that preserves the task withdraws the close it recorded --------
#
# The pending-close record exists for a CRASH mid-cleanup, which the case above
# pins. A refusal is not a crash: cleanup that stops on purpose to keep a task
# it could not archive must also withdraw that close, or the next session start
# replays it and removes exactly the record the refusal preserved.
test_a_preserving_refusal_withdraws_the_close_it_recorded() {
  local case_dir home id marker out rc=0
  id=atomic-close-preserving-refusal-b9
  case_dir=$(make_home close-preserving-refusal "$id")
  home=$(home_of "$case_dir")
  add_item "$case_dir" "$id"
  out=$(run_ship_spawn "$case_dir" "$id") || fail "spawn failed: $out"
  marker="$home/state/$id.backlog-close"
  # An unwritable manifest destination, so publication fails after the pending
  # close has already been recorded.
  mkdir -p "$home/data/$id/outcome.json"

  out=$(run_teardown "$case_dir" "$id") || rc=$?
  [ "$rc" -ne 0 ] || fail "teardown succeeded though its completion manifest could not be published"
  assert_contains "$out" "could not publish the durable outcome manifest" \
    "teardown did not name the manifest failure: $out"
  assert_present "$home/state/$id.meta" \
    "the refusal erased the task record it exists to preserve"
  assert_absent "$marker" \
    "a refusal that preserved the task left a pending close a restart would replay"

  out=$(run_bootstrap "$case_dir")
  [ "$(row_state "$case_dir" "$id")" = in_flight ] \
    || fail "restart closed a preserved task's item anyway: $out"
  assert_present "$home/state/$id.meta" \
    "restart removed the record the refusal preserved: $out"
  pass "a refusal that preserves the task withdraws its recorded close"
}
