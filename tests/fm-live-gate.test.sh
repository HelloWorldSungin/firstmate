...
# requires the shared refusal line for upstream guards and the explicit named
# refusal for fork-only guards whose stronger execution opt-ins remain in force. It is
...
test_every_live_guard_is_wired_to_the_shared_gate() {
  local script out listing expected checked=0
  ...
  while IFS= read -r script; do
    ...
    expected="skip: live: disabled by FM_LIVE=0"
    case "$(basename "$script")" in
      fm-afk-pi-dual-supervision-e2e.test.sh) expected="skip: set FM_AFK_PI_DUAL_SUPERVISION_E2E=1" ;;
    ...
    assert_contains "$out" "$expected" \
      "$(basename "$script") must refuse execution through its owning gate"
    checked=$((checked + 1))
  done <<EOF
$listing
EOF
...
test_ordinary_ci_preserves_protected_execution_gates() {
  local script expected out serial
  serial=$("$ROOT/bin/fm-test-run.sh" --list --lane portable-serial)
  while IFS=' ' read -r script expected; do
    printf '%s\n' "$serial" | grep -Fxq "tests/$script" \
      || fail "$script must be inventoried in portable serial"
    out=$(clean_env CI=true bash "$ROOT/tests/$script" 2>&1) \
      || fail "$script must skip successfully in ordinary CI: $out"
    assert_contains "$out" "$expected" "$script must retain its execution opt-in in CI"
  done <<'CASES'
...
fm-codex-continuity-live-e2e.test.sh skip: live: opt-in;
CASES
  pass "ordinary CI inventories protected guards without executing their live workloads"
}
