...
case "${1:-} ${2:-}" in
  "pr view")
    ...
    ;;
  "pr merge") : > "$(dirname "$0")/../gh-merge-called"; printf 'merged:\n  number: %s\n  status: ok\n' "${3:-}" ;;
  "api graphql")
    if [ -e "$(dirname "$0")/../gh-merge-called" ]; then
...
test_design_teardown_requires_inventory_verification() {
  ...
  if run_teardown "$home" "$id" > "$home/design-teardown.out" 2> "$home/design-teardown.err"; then
    fail "design teardown skipped captain-call inventory verification"
  fi
  grep -F 'design task sample-adr-design has not passed the captain-call completion gate' \
    "$home/design-teardown.err" >/dev/null \
    || fail "design teardown did not identify its captain-call inventory refusal"
  [ -e "$home/state/$id.meta" ] \
    || fail "refused design teardown removed metadata"
  pass "non-forced design teardown requires durable captain-call inventory verification"
}
