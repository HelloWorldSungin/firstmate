...
#   (q) a detached worktree is the ship's pre-branch window, in which no run of
#       its own can exist, so it spends no bounded lookup at all - while the one
#       state that can hold an attributable run still performs that lookup
#   (r) coarse runs-ledger fallback: a terminal failed record with the daemon
...
exit 0
SH
  # Match the synthetic shell process-info with a complete, isolated terminal snapshot.
  cat > "$fb/herdr-ps" <<'SH'
#!/usr/bin/env bash
printf '%s 1 S zsh fmfixture\n' "$FM_FAKE_HERDR_SHELL_PID"
SH
  chmod +x "$fb/herdr-ps"
...
run_crew_state() {  # <case-dir> <id>
  PATH="$1/fakebin:$PATH" FM_STATE_OVERRIDE="$1/state" \
    FM_HERDR_PS_BIN="$1/fakebin/herdr-ps" "$CREW_STATE" "$2"
}
...
reset_fakes() {
  ...
  export FM_FAKE_DAEMON_DOWN FM_FAKE_TMUX_UNREADABLE FM_FAKE_HERDR_READ_FAIL FM_FAKE_HERDR_HUSK
...
# The detached window is the ship's pre-branch phase: the brief creates the
# recorded branch before any validation, so while HEAD is detached no run of
# this task's can exist to attribute. Reading state there must cost zero bounded
# no-mistakes calls per heartbeat, however loudly `axi status` would answer.
test_detached_worktree_skips_the_run_lookup() {
  reset_fakes
  local d out; d=$(new_case detached-skip)
  make_repo_on_branch "$d/wt" fm/detached-task
  git -C "$d/wt" checkout --detach -q
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/detached-task.meta" "window=fm:fm-detached-task" "worktree=$d/wt" \
    "branch=fm/detached-task" "kind=ship" "harness=claude"
...
EOF
  ...
  FM_FAKE_BUSY=0
  arm_idle_record "$d/state" detached-task
  FM_FAKE_NM_CALL_LOG="$d/nm-calls.log"
  : > "$FM_FAKE_NM_CALL_LOG"
  out=$(run_crew_state "$d" detached-task)
  [ ! -s "$FM_FAKE_NM_CALL_LOG" ] \
    || fail "a detached worktree still spent bounded no-mistakes calls: $(cat "$FM_FAKE_NM_CALL_LOG")"
  assert_contains "$out" "state: blocked" "detached crew lost its recorded blocked state"
  assert_contains "$out" "source: status-log" "detached crew did not fall through to its own log"
...
test_recorded_branch_checked_out_still_runs_the_lookup() {
  reset_fakes
  local d out; d=$(new_case attached-lookup)
  make_repo_on_branch "$d/wt" fm/attached-task
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/attached-task.meta" "window=fm:fm-attached-task" "worktree=$d/wt" \
    "branch=fm/attached-task" "kind=ship" "harness=claude"
  printf 'blocked: worktree isolation is not verified\n' > "$d/state/attached-task.status"
  FM_FAKE_AXI_STATUS="$(run_running fm/attached-task)"
  FM_FAKE_BUSY=0
  arm_idle_record "$d/state" attached-task
  FM_FAKE_NM_CALL_LOG="$d/nm-calls.log"
  : > "$FM_FAKE_NM_CALL_LOG"
  out=$(run_crew_state "$d" attached-task)
  grep -Fxq "axi status" "$FM_FAKE_NM_CALL_LOG" \
    || fail "the short-circuit skipped a crew whose recorded branch is checked out"
  [ "$out" = "state: working · source: run-step · validating (running) · status-log superseded by active run" ] \
    || fail "an attributable run stopped being the answer: $out"
  pass "a worktree on its recorded branch still performs the run lookup"
}
...
# Breaks if captain-held is not treated like paused when a terminal run-step
...
test_captain_held_over_forge_skipped_pass_reports_the_wait() {
  ...
  assert_not_contains "$out" "state: done" "the terminal pass must not outrank captain-held"
  pass "captain-held over a forge-unobserved terminal pass reports the wait"
}

# Breaks if checks-green terminal runs (ci observed, PR still open) still read
...
test_captain_held_over_checks_green_terminal_reports_the_wait() {
  ...
  assert_not_contains "$out" "state: done" "checks-green terminal must not outrank captain-held"
  pass "captain-held over a checks-green terminal run reports the wait"
}
...
test_captain_held_over_cancelled_run_reports_the_wait() {
  ...
  assert_not_contains "$out" "state: failed" "cancelled terminal must not outrank captain-held"
...
test_captain_held_over_observed_merge_stays_done() {
  ...
  pass "captain-held over an observed merge does not override terminal done"
}

test_degraded_observed_merge_suppresses_stale_wait() {
  reset_fakes
  local d out; d=$(new_case degraded-observed-merge-held)
  make_repo_on_branch "$d/wt" fm/feat-degraded-merge-held
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/degraded-merge-held.meta" "window=fm:fm-degraded-merge-held" \
    "worktree=$d/wt" "kind=ship"
  printf 'done: merged\n' > "$d/state/degraded-merge-held.status"
  FM_FAKE_AXI_STATUS="$(run_passed_forge_observed fm/feat-degraded-merge-held)"
  out=$(run_crew_state "$d" degraded-merge-held)
  assert_contains "$out" "state: done" "the observed merge seeds terminal evidence"
  printf 'captain-held: stale hold after merge\n' >> "$d/state/degraded-merge-held.status"
  FM_FAKE_NM_RC=124
  out=$(run_crew_state "$d" degraded-merge-held)
  assert_contains "$out" "state: done" "degraded merge evidence remains authoritative"
  assert_contains "$out" "source: run-step-degraded" "the unavailable lookup replays merge evidence"
  assert_not_contains "$out" "state: paused" "a stale wait cannot mask a degraded observed merge"
  pass "degraded replay preserves the observed-merge exception"
}

# Breaks if declared waits outrank still-advancing runs, letting a crew park
# its way out of a live validation gate.
test_declared_wait_over_active_run_stays_run_step() {
  reset_fakes
  local d; d=$(new_case active-run-captain-held)
  make_repo_on_branch "$d/wt" fm/feat-active-held
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/feat-active-held.meta" "window=fm:fm-feat-active-held" "worktree=$d/wt" "kind=ship"
  printf 'captain-held: should not override active validation\n' > "$d/state/feat-active-held.status"
  FM_FAKE_AXI_STATUS="$(run_running fm/feat-active-held)"
  local out; out=$(run_crew_state "$d" feat-active-held)
  assert_contains "$out" "state: working" "an active run stays working"
  assert_contains "$out" "source: run-step" "the active run remains run-step sourced"
...
EOF
  ...
  FM_FAKE_AXI_STATUS="$(run_running fm/feat-same-head-rerun 01RUN-B)"
  FM_FAKE_RUNS_LIST=""
  out=$(run_crew_state "$d" same-head-rerun)
  assert_contains "$out" "state: working" "the rerun first establishes its own active boundary"
...
test_unseen_same_head_rerun_rejects_prior_generation_park() {
  ...
  FM_FAKE_AXI_STATUS="$(run_passed_forge_skipped fm/feat-unseen-same-head-rerun 01K00000000000000000000000)"
  ...
  pass "same-head reruns use exact generation identity"
}

test_terminal_rerun_parked_before_first_read_reports_the_wait() {
  reset_fakes
  local d out; d=$(new_case terminal-rerun-first-read-park)
  make_repo_on_branch "$d/wt" fm/feat-terminal-rerun-first-read
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/terminal-rerun-first-read.meta" \
    "window=fm:fm-terminal-rerun-first-read" "worktree=$d/wt" "kind=ship"
  printf 'done: first run finished locally\n' > "$d/state/terminal-rerun-first-read.status"
  FM_FAKE_AXI_STATUS="$(run_passed_forge_skipped fm/feat-terminal-rerun-first-read 01K00000000000000000000000)"
  out=$(run_crew_state "$d" terminal-rerun-first-read)
  assert_contains "$out" "state: done" "the prior generation establishes its terminal record"
  printf 'captain-held: second run finished, awaiting release approval\n' \
    >> "$d/state/terminal-rerun-first-read.status"
  FM_FAKE_AXI_STATUS="$(run_passed_forge_skipped fm/feat-terminal-rerun-first-read 01K00000000000000000000001)"
  out=$(run_crew_state "$d" terminal-rerun-first-read)
  assert_contains "$out" "state: paused" "a terminal rerun honors its park on first observation"
  assert_contains "$out" "awaiting release approval" "the rerun park keeps its reason"
  out=$(run_crew_state "$d" terminal-rerun-first-read)
  assert_contains "$out" "state: paused" "the rerun park remains ordered after its first read"
  pass "first-observed terminal reruns preserve their declared park"
}
...
test_coarse_abandoned_run_keeps_terminal_identity() {
  reset_fakes
  local d short out; d=$(new_case coarse-abandoned-identity)
  make_repo_on_branch "$d/wt" fm/feat-coarse-abandoned
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/coarse-abandoned.meta" "window=fm:fm-coarse-abandoned" \
    "worktree=$d/wt" "kind=ship"
...
EOF
  ...
  out=$(run_crew_state "$d" coarse-abandoned)
  assert_contains "$out" "state: failed" "the terminal row retains the abandoned run's generation"
  assert_not_contains "$out" "state: paused" "the pre-run hold cannot mask the abandoned run's failure"
  pass "coarse abandoned verdicts retain terminal identity"
}
...
test_coarse_completed_preserves_observed_merge() {
  ...
  FM_FAKE_AXI_STATUS="$(run_passed_forge_observed fm/feat-coarse-observed-merge 01KZY5YSHPXCBS6W7G7XC8WBK0)"
...
EOF
  ...
  grep -Fxq 'axi' "$FM_FAKE_NM_CALL_LOG" \
    && fail "merge evidence depended on the capped AXI home table"
...
EOF
  ...
  FM_FAKE_RUNS_LIST="running fm/feat-nosibling ${short_base} 2026-08-05 11:20"
  FM_FAKE_RUNS_RC=124
  out=$(run_crew_state "$d" nosibling)
  assert_contains "$out" "run failed" "partial output from a failed ledger probe must not replace full terminal evidence"
...
test_missing_run_head_falls_back_to_current_state() {
  reset_fakes
  local d out
  d=$(new_case missing-run-head-current-state)
  make_repo_on_branch "$d/wt" fm/feat-no-head
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/no-head.meta" "window=fm:fm-no-head" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'working: current stage still in progress\n' > "$d/state/no-head.status"
...
# The approved ledger fallback needs a terminal exact-HEAD anchor. Each case
# starts with fresh task state so degraded replay cannot mask a rejected row.
test_unfetched_run_requires_terminal_exact_anchor() {
  local variant d h0 h1 h2 h3 anchor_status anchor_branch anchor_head out expected row_day row_clock
  for variant in completed failed cancelled running fixing queued unknown missing foreign-head foreign-branch dead-worker foreign-detail terminal-detail malformed-row impossible-date invalid-leap valid-leap invalid-clock malformed-anchor; do
    reset_fakes
    d=$(new_case "terminal-anchor-$variant")
    make_repo_on_branch "$d/wt" fm/terminal-anchor
    h0=$(git -C "$d/wt" rev-parse HEAD)
    git -C "$d/wt" commit -q --allow-empty -m submitted
    h1=$(git -C "$d/wt" rev-parse HEAD)
    h2=$(mint_unfetched_fix_head "$d/wt")
    make_fakebin "$d" >/dev/null
    fm_write_meta "$d/state/anchor.meta" "window=fm:fm-anchor" "worktree=$d/wt" "kind=ship" "branch=fm/terminal-anchor" "harness=claude"
    printf '%s\n' 'paused: waiting for validation ownership' > "$d/state/anchor.status"
    arm_idle_record "$d/state" anchor
    FM_FAKE_RUN_HEAD=$h2
    FM_FAKE_AXI_STATUS="$(run_fixing fm/terminal-anchor)"
    anchor_status=$variant
    anchor_branch=fm/terminal-anchor
    anchor_head=$h1
    row_day=2026-09-11
    row_clock=01:00
    expected=paused
    case "$variant" in
      completed|failed|cancelled) expected=working ;;
      foreign-head) anchor_status=failed; anchor_head=$h0 ;;
      foreign-branch) anchor_status=failed; anchor_branch=fm/other ;;
      dead-worker) anchor_status=failed; FM_FAKE_AGENT_STATE=dead; expected=abandoned ;;
      malformed-row|malformed-anchor) anchor_status=failed ;;
      impossible-date) anchor_status=failed; row_day=2026-02-31 ;;
      invalid-leap) anchor_status=failed; row_day=2026-02-29 ;;
      valid-leap) anchor_status=failed; row_day=2024-02-29; expected=working ;;
      invalid-clock) anchor_status=failed; row_clock=25:00 ;;
      terminal-detail)
        anchor_status=failed; expected=working
        FM_FAKE_AXI_STATUS="$(run_passed fm/terminal-anchor)"
        ;;
      foreign-detail)
        anchor_status=failed; expected=working
        git -C "$d/wt.pipe" commit -q --allow-empty -m other-unfetched-run
        h3=$(git -C "$d/wt.pipe" rev-parse HEAD)
        FM_FAKE_RUN_HEAD=$h3
        FM_FAKE_AXI_STATUS="$(run_fixing fm/terminal-anchor)"
        ;;
    esac
    FM_FAKE_RUNS_LIST="running fm/terminal-anchor ${h2:0:7} $row_day $row_clock"
    if [ "$variant" = malformed-row ]; then
      FM_FAKE_RUNS_LIST="running fm/terminal-anchor ${h2:0:7}"
    fi
    if [ "$variant" != missing ]; then
      FM_FAKE_RUNS_LIST="$FM_FAKE_RUNS_LIST
$anchor_status $anchor_branch ${anchor_head:0:7} $row_day 00:00"
    fi
    if [ "$variant" = malformed-anchor ]; then
      FM_FAKE_RUNS_LIST="$FM_FAKE_RUNS_LIST unexpected-field"
    fi
    out=$(run_crew_state "$d" anchor)
    assert_contains "$out" "state: $expected" "terminal-anchor $variant produced the wrong worker state"
    if [ "$expected" = paused ]; then
      assert_contains "$out" 'source: status-log' "terminal-anchor $variant must not bind an unproven run"
    else
      assert_contains "$out" 'source: run-step' "terminal-anchor $variant did not attribute the selected run"
    fi
    if [ "$variant" = terminal-detail ]; then
      assert_not_contains "$out" 'local pipeline passed' 'a stale terminal AXI result overrode the newest running ledger row'
    fi
    if [ "$variant" = foreign-detail ]; then
      assert_not_contains "$out" 'validating (fixing)' 'a different axi head lent its full detail to the selected ledger row'
    fi
  done
  pass "unfetched runs require terminal exact-HEAD same-branch anchors and preserve worker liveness and run-detail identity"
}

test_malformed_ledger_preserves_degraded_evidence() {
  reset_fakes
  local d head out
  d=$(new_case malformed-ledger-cache)
  make_repo_on_branch "$d/wt" fm/malformed-cache
  head=$(git -C "$d/wt" rev-parse HEAD)
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/cache.meta" "window=fm:fm-cache" "worktree=$d/wt" \
    "kind=ship" "branch=fm/malformed-cache" "harness=claude"
  printf 'paused: waiting for validation\n' > "$d/state/cache.status"
  arm_idle_record "$d/state" cache
  FM_FAKE_RUN_HEAD=$head
  FM_FAKE_AXI_STATUS="$(run_running fm/malformed-cache)"
  out=$(run_crew_state "$d" cache)
  assert_contains "$out" 'source: run-step' "valid run did not seed observed evidence"
  FM_FAKE_AXI_STATUS="$(run_running fm/other)"
  FM_FAKE_RUNS_LIST="running fm/malformed-cache ${head:0:7} 2026-02-31 00:00"
  out=$(run_crew_state "$d" cache)
  assert_contains "$out" 'source: run-step-degraded' "malformed ledger erased the last observed step"
  assert_contains "$out" 'runs ledger malformed' "degradation did not identify the invalid input"
  FM_FAKE_RUNS_LIST="running fm/other ${head:0:7} 2026-09-11 00:00"
  out=$(run_crew_state "$d" cache)
  assert_contains "$out" 'source: status-log' "confirmed branch absence kept replaying old evidence"
  assert_contains "$out" 'state: paused' "confirmed absence lost the declared wait"
  pass "malformed ledger preserves bounded degraded evidence while confirmed absence clears it"
}
