...
test_unfetched_run_requires_terminal_exact_anchor() {
  local variant d h0 h1 h2 h3 anchor_status anchor_branch anchor_head out expected
  for variant in completed failed cancelled running fixing queued unknown missing foreign-head foreign-branch dead-worker foreign-detail terminal-detail; do
    reset_fakes
    ...
    FM_FAKE_RUNS_LIST="running fm/terminal-anchor ${h2:0:7} 2026-09-11 01:00"
    if [ "$variant" != missing ]; then
      FM_FAKE_RUNS_LIST="$FM_FAKE_RUNS_LIST
$anchor_status $anchor_branch ${anchor_head:0:7} 2026-09-11 00:00"
    fi
