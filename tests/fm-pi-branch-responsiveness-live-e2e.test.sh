...
# directory, disposable HOME and agent directory, --approve, and --offline.
# An explicit built-in model selects a displayable TUI without credentials;
# extension discovery is disabled in every arm and only the measured extension
# is loaded explicitly. NO model turn is started: /new is a local command and re-fires session_start,
...
mkdir -p "$PROJECT/.pi/extensions/lib" "$HOME_DIR/state" "$HOME_DIR/config" "$SESSION_DIR" "$USER_HOME" "$AGENT_DIR"
...
exec env -i PATH="$PATH" HOME="$LAB_USER_HOME" TERM="${TERM:-xterm-256color}" \
  LANG="${LANG:-C.UTF-8}" PI_CODING_AGENT_DIR="$LAB_AGENT_DIR" \
  FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$FM_STATE_OVERRIDE" \
  FM_CONFIG_OVERRIDE="$FM_CONFIG_OVERRIDE" FM_ROOT_OVERRIDE="$FM_ROOT_OVERRIDE" "$@"
SH
...
run_arm() {
  ...
  local -a pi_args
  pi_args=(pi -ne -ns -np -nc --no-themes --offline --approve \
    --provider openai --model gpt-4o --session-dir "$SESSION_DIR")
  if [ "$load_extension" = yes ]; then
    pi_args+=(-e "$EXT")
  fi
  "$TMUX" -L "$SOCKET" -f /dev/null new-session -d -s "$SESSION" -x 200 -y 50 \
    "FM_HOME='$HOME_DIR' FM_STATE_OVERRIDE='$HOME_DIR/state' FM_CONFIG_OVERRIDE='$HOME_DIR/config' \
     FM_ROOT_OVERRIDE='$ROOT' LAB_PROJECT='$PROJECT' LAB_USER_HOME='$USER_HOME' \
     LAB_AGENT_DIR='$AGENT_DIR' '$LAB/launch.sh' ${pi_args[*]@Q}"
