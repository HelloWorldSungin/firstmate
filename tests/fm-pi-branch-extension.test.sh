...
// Compatibility with the unlanded detector's authored-answer shape only.
// This does not import that detector or treat a tag as proof of authorship.
writeFileSync(`${state}/task-a.status`,
  "needs-decision [key=cleanup]: choose cleanup\n" +
  "resolved [key=other] [by=firstmate]: answered: unrelated\nworking: resumed\n");
const wrongAuthoredKey = scopeForUnreadWake(state, false);
if (wrongAuthoredKey.eligibleSeqs.join(",") !== "2" ||
  wrongAuthoredKey.needsDecisionKeys.join(",") !== "fm-window") {
  throw new Error(`an authored answer for another key closed the decision: ${JSON.stringify(wrongAuthoredKey)}`);
}
writeFileSync(`${state}/task-a.status`,
  "needs-decision [key=cleanup]: choose cleanup\n" +
  "resolved [key=cleanup] [by=firstmate]: answered: preserve strict identity\nworking: resumed\n");
const authoredResolution = scopeForUnreadWake(state, false);
if (authoredResolution.eligibleSeqs.join(",") !== "1,2" || authoredResolution.needsDecisionKeys.length !== 0) {
  throw new Error(`authored resolution metadata left the matching decision open: ${JSON.stringify(authoredResolution)}`);
}
