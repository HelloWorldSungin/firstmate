...
trap 'FM_HOME="$PARENT" FM_PROCEVENT_CLAIM_ROOT="$CLAIMS" "$ROOT/bin/fm-procevent.sh" sweep-home >/dev/null 2>&1 || true; fm_test_stop_remote_job_worker "$TMP_ROOT/remote-jobs"; rm -rf -- "$TMP_ROOT"' EXIT
