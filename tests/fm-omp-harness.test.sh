...
test_watch_extension_respects_away_daemon_ownership() {
  local repo home out status
  repo="$TMP_ROOT/watch-away/repo"; home="$TMP_ROOT/watch-away/home"
  install_omp_extension_fixture "$repo"
  mkdir -p "$home/state"
  cat > "$repo/bin/fm-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
...
printf '%s\n' "$$" >> "$FM_HOME/state/starts"
printf 'watcher: started pid=%s (beacon 0s) recovery-generation=gen-1\n' "$$"
if [ ! -e "$FM_HOME/state/reason-ready" ]; then
  printf 'signal: omp-away-pending done\n'
  : > "$FM_HOME/state/reason-ready"
fi
exec sleep 30
SH
  chmod +x "$repo/bin/fm-watch-arm.sh"
  out=$(FM_HOME="$home" FM_STATE_OVERRIDE="$home/state" FM_ROOT_OVERRIDE="$repo" \
    FM_OMP_AWAY_POLL_MS=20 FM_WATCH_ARM_RETIRE_TIMEOUT_MS=100 \
    EXT="$repo/.omp/extensions/fm-primary-omp-watch.ts" node --input-type=module 2>&1 <<'EOF'

import {pathToFileURL} from 'node:url';
import {writeFileSync,readFileSync,existsSync,unlinkSync} from 'node:fs';
const state=process.env.FM_HOME+'/state';
writeFileSync(state+'/.lock',process.pid+'\n');writeFileSync(state+'/.afk','');
const handlers=new Map();let tool;const sent=[];
const api={on(e,h){handlers.set(e,h)},registerCommand(){},registerTool(t){tool=t},sendUserMessage(m){sent.push(m)}};
const mod=await import(pathToFileURL(process.env.EXT).href);mod.default(api);
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
const starts=()=>existsSync(state+'/starts')?readFileSync(state+'/starts','utf8').trim().split('\n').filter(Boolean):[];
const alive=p=>{try{process.kill(Number(p),0);return true}catch{return false}};
async function until(pred,note){for(let i=0;i<200;i++){if(pred())return;await sleep(20)}throw new Error(note)}
try {
 const result=await tool.execute();
 if(!result.content[0].text.includes('standby'))throw new Error('away launch started a watcher instead of standing by: '+result.content[0].text);
 await sleep(80);if(starts().length)throw new Error('away mode spawned a child');
 unlinkSync(state+'/.afk');await until(()=>starts().length===1,'return did not resume one child');
 await until(()=>existsSync(state+'/reason-ready'),'first child did not publish its reason');await sleep(40);
 const first=starts()[0];writeFileSync(state+'/.afk','');await until(()=>!alive(first),'away transition did not retire child');
 if(sent.length)throw new Error('away mode delivered an ordinary wake');
 unlinkSync(state+'/.afk');await until(()=>starts().length===2,'second return did not resume one child');
 await until(()=>sent.length===1,'pending close was not delivered on return');
 if(!sent[0].includes('signal: omp-away-pending done'))throw new Error('pending close text changed');
 await handlers.get('before_agent_start')({prompt:sent[0]},{});
 await tool.execute();await sleep(80);if(starts().length!==2)throw new Error('return created duplicate watcher');
 writeFileSync(state+'/.afk','');await handlers.get('session_shutdown')({},{});
 await handlers.get('session_start')({},{});await sleep(80);if(starts().length!==2)throw new Error('replacement armed while away');
 unlinkSync(state+'/.afk');await until(()=>starts().length===3,'replacement return did not resume');
 if(sent.length!==1)throw new Error('pending close was duplicated');
 console.log('ok - OMP away standby retains pending delivery, retires its child, resumes one cycle and survives replacement');
} finally {await handlers.get('session_shutdown')({},{});}
await until(()=>starts().every(p=>!alive(p)),'probe left a watcher child');
EOF
)
  status=$?
  expect_code 0 "$status" "OMP away ownership and pending-delivery contract: $out"
  pass ".omp watcher stands down for the away daemon, preserves pending delivery, and resumes one cycle across replacement"
}
...
test_watch_extension_respects_away_daemon_ownership
printf '\nall fm-omp-harness tests passed\n'
