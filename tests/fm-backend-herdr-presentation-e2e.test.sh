...
if grep -F "target is the captain's active tab" "$EVIDENCE_ROOT/active-seeded.err" >/dev/null 2>&1; then
...
teardown_task lock-contended "$HOME_DIR" > "$EVIDENCE_ROOT/lock-contended-teardown.out" 2> "$EVIDENCE_ROOT/lock-contended-teardown.err" \
  || fail "flat lock-contention fixture teardown failed: $(cat "$EVIDENCE_ROOT/lock-contended-teardown.err")"
