...
case "${1:-} ${2:-}" in
  "api graphql")
    if [ ! -f "$(dirname "$0")/../gh-merge-called" ]; then
      printf '%s\n' state=OPEN merged=false "queued=${FM_TEST_GH_GRAPHQL_QUEUED:-false}" base=main default=main
      exit 0
    fi
    printf '%s\n' \
...
case "${1:-} ${2:-}" in
  ...
  api\ *)
    ...
    case " $* " in
      *'{base:'*) ;;
      *'{tip:'*) printf 'tip: 1111111111111111111111111111111111111111\n'; exit 0 ;;
