...
test_quiet_exit_has_no_away_catchup() {
  local dir out
  dir="$TMP_ROOT/quiet-mode-return"
  install_runner "$dir"
  printf 'quiet\n%s\n' "$(date +%s)" > "$dir/home/state/.afk"
  out=$(run_return "$dir" guard) || fail "quiet ordinary work guard failed: $out"
  [ -e "$dir/home/state/.afk" ] || fail "quiet ordinary work guard cleared mode"
  out=$(run_return "$dir" quiet-off) || fail "quiet-off failed: $out"
  [ ! -e "$dir/home/state/.afk" ] || fail "quiet exit left the mode flag"
  [ ! -e "$dir/home/state/.afk-return-catchup" ] || fail "quiet exit fabricated an away gate"
  [ ! -d "$dir/home/state/afk-contracts" ] || fail "quiet exit fabricated an away archive"
  [ "$(wc -l < "$dir/home/stop.log" | tr -d ' ')" -eq 1 ] || fail "quiet exit did not stop the daemon exactly once"
  pass "explicit quiet exit stops the shared daemon without an away catch-up"
}
