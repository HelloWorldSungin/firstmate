...
test_non_ascii_blank_pad_is_empty() {
  ...
  # Every entry here has the Unicode property White_Space=Yes, which is the
  # property FM_COMPOSER_UNICODE_SPACES follows. Zero-width format characters
  # (U+200B, U+FEFF) are deliberately NOT folded: they carry White_Space=No, and
  # a pane padded with one reads `pending`, which defers injection and raises the
  # wedge alarm rather than injecting on an unproven verdict. The live harness
  # guard is what catches a harness that starts drawing composers with one.
  for b in $'\xc2\xa0' $'\xe2\x80\x87' $'\xe2\x80\xaf' $'\xe3\x80\x80'; do
    out=$(classify 0 "❯${b}")
    [ "$out" = empty ] \
...
test_multibyte_agent_glyph_is_stripped_whole() {
  ...
  # A proven placeholder position in a plain box capture, which is where the
  # shared classifier now permits idle matching at all; the point here stays the
  # glyph strip, so the remainder must match the idle pattern byte-exactly.
  out=$( export LC_ALL=C; classify 1 '❯ Type a message...' '^Type a message\.\.\.$' sensitive '❯ Type a message...' 1 0 )
  [ "$out" = empty ] \
