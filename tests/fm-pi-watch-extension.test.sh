...
# The marker is published by rename too: currentArm() reads the pid file and
# then this one, so a truncate-then-write here could hand a reader the previous
# pid beside an empty marker.
printf '%s\n' "$marker" > "${FM_CHILD_MARKER_FILE:?}.$$"
mv -f "${FM_CHILD_MARKER_FILE:?}.$$" "${FM_CHILD_MARKER_FILE:?}"
printf 'arm pid=%s marker=%s\n' "$$" "$marker" >> "${FM_ARM_LOG:?}"
# The pid file is the single commit point the assertions poll on, so it is
# published last and by rename. Appending the arm log or the marker after it, or
# letting a truncate-then-write expose an empty file, would let a reader observe
# this child in the pid file while the live-arm log still reads as none.
printf '%s\n' "$$" > "${FM_CHILD_PID_FILE:?}.$$"
...
test_pi_streaming_followup_is_replayed_after_replacement() {
  local repo home plugin trigger out status replacement_mode=${1:-active}
  repo="$TMP_ROOT/pi-streaming-followup-replacement-$replacement_mode-root"
  home="$TMP_ROOT/pi-streaming-followup-replacement-$replacement_mode-home"
  trigger="$TMP_ROOT/pi-streaming-followup-replacement-$replacement_mode.trigger"
  mkdir -p "$repo/bin" "$home/state" "$home/config"
  install_pi_watch_extension_fixture "$repo"
  plugin="$repo/.pi/extensions/fm-primary-pi-watch.ts"
  cat > "$repo/bin/fm-watch-arm.sh" <<'SH'
...
SH
  chmod +x "$repo/bin/fm-watch-arm.sh"
  out=$(PLUGIN="$plugin" FM_HOME="$home" FM_ROOT_OVERRIDE="$repo" FM_TRIGGER_FILE="$trigger" FM_ARM_LOG="$home/arm.log" \
    FM_REPLACEMENT_MODE="$replacement_mode" FM_PI_AWAY_POLL_MS=50 node --input-type=module 2>&1 <<'EOF'
...
const awayFlag = `${process.env.FM_HOME}/state/.afk`;
const awayReplacement = process.env.FM_REPLACEMENT_MODE === "away";
const armCount = () => existsSync(process.env.FM_ARM_LOG)
  ? readFileSync(process.env.FM_ARM_LOG, "utf8").trim().split("\n").length : 0;
const retiredArmCount = armCount();
if (awayReplacement) writeFileSync(awayFlag, "away\n");
...
if (awayReplacement) {
  ...
  if (replacement.prompts.length !== 0 || armCount() !== retiredArmCount) {
    throw new Error("away-mode replacement delivered a pending wake or armed beside the away supervisor");
  }
  rmSync(awayFlag);
}
await waitFor(
...
EOF
  ...
  pass "Pi replacement replays a streaming follow-up before consumption ($replacement_mode replacement)"
}
