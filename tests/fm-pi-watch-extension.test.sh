...
test_spawn_template_mentions_pi_watch_placeholder() {
  local text
  # The launch TEMPLATE lives in bin/fm-launch-lib.sh (fm_launch_template); the
  # placeholder SUBSTITUTION and the tracked-extension path stay in bin/fm-spawn.sh.
  # Read both so the assertions cover the split.
  text=$(cat "$ROOT/bin/fm-spawn.sh" "$ROOT/bin/fm-launch-lib.sh")
