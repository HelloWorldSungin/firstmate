...
exit 2
SH
  # Records the backoff schedule instead of spending it, so the assertion below
  # pins the widened wait budget without making this test slow.
  cat > "$fakebin/sleep" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$1" >> "$SLEEP_WAITS"
exit 0
SH
  ...
  # Pin the platform so the stubbed checksum above is the archive the
  # installer selects, whatever host this suite runs on.
  fm_install_stub_uname "$fakebin"

  out=$(FM_TEST_UNAME_S=Linux FM_TEST_UNAME_M=x86_64 CURL_COUNT="$tmp/curl-count" CURL_ARGS="$tmp/curl-args" SLEEP_WAITS="$tmp/sleep-waits" \
    PATH="$fakebin:$PATH" "$INSTALLER" "$destination" 2>&1) \
...
SH
  ...
  rc=0
  out=$(FM_TEST_UNAME_S=Linux FM_TEST_UNAME_M=x86_64 CURL_COUNT="$tmp/curl-count" SLEEP_WAITS="$tmp/sleep-waits" \
