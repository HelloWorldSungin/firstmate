...
# A terminal exact-HEAD ledger anchor can support read-only current-state
# attribution, but Firstmate retained strict object-local identity for abort.
# An unfetched head therefore remains untouched even with that valid anchor.
test_unfetched_parked_run_never_uses_ledger_abort_authority() {
...
EOF
  ...
  assert_absent "$case_dir/nm-abort.log" \
    "unfetched parked run gained destructive authority from read-only ledger evidence"
  assert_absent "$case_dir/nm-runs.log" \
    "strict teardown unexpectedly queried the ledger to grant abort authority"
  assert_no_grep "parked at a gate; aborting" "$case_dir/stderr" \
    "strict teardown claimed to abort an unfetched run"
  pass "an unfetched parked run remains untouched even with a terminal exact-HEAD ledger anchor"
}
...
EOF
  ...
  assert_absent "$case_dir/nm-runs.log" \
    "parked-run-impossible-ledger-date: strict teardown must refuse before any ledger fallback"
  assert_absent "$case_dir/nm-abort.log" \
...
EOF
  ...
  assert_absent "$case_dir/nm-runs.log" \
    "parked-run-terminal-newest-at-head: strict teardown must refuse before any ledger fallback"
  assert_absent "$case_dir/nm-abort.log" \
