...
# Exercise the real spawn entrypoint with a deterministic Herdr adapter.
# The transport fake below owns the screen transitions; native backend protocol
# and atomic delivery have separate coverage in fm-agy-adapter.test.sh.
SPAWN_ROOT="$TMP_ROOT/spawn-runtime"
mkdir -p "$SPAWN_ROOT"
cp -R "$ROOT/bin" "$SPAWN_ROOT/bin"
cp "$ROOT/.tasks.toml" "$SPAWN_ROOT/"
SPAWN="$SPAWN_ROOT/bin/fm-spawn.sh"
cat > "$SPAWN_ROOT/bin/backends/herdr.sh" <<'SH'
#!/usr/bin/env bash
fm_backend_herdr_agent_prompt_capability_check() { return 0; }
fm_backend_herdr_agent_prompt_version_check() { return 0; }
fm_backend_herdr_presentation_enabled() { return 1; }
fm_backend_herdr_projection_journal_path() { printf '%s/%s.herdr-presentation\n' "$1" "$2"; }
fm_backend_herdr_workspace_label() { printf firstmate; }
fm_backend_herdr_session() { printf fixture; }
fm_backend_herdr_container_ensure() { printf 'fixture:w1\t\n'; }
fm_backend_herdr_create_task() { printf 't1 w1:p1\n'; }
fm_backend_herdr_current_path() { printf '%s\n' "$FM_FAKE_PANE_PATH"; }
fm_backend_herdr_parse_target() { printf 'fixture w1:p1\n'; }
fm_backend_herdr_capture() { tmux capture-pane -p; }
fm_backend_herdr_capture_ansi() { tmux capture-pane -p; }
fm_backend_herdr_send_literal() { tmux send-keys -t "$1" -l "$2"; }
fm_backend_herdr_send_key() { tmux send-keys -t "$1" "$2"; }
fm_backend_herdr_send_text_line() { tmux send-keys -t "$1" -l "$2"; }
fm_backend_herdr_kill() { tmux kill-window -t "$1"; }
fm_backend_herdr_busy_state() {
  case "$(cat "$FM_FAKE_AGY_STATE")" in busy|racing) printf busy ;; *) printf unknown ;; esac
}
fm_backend_herdr_agent_identity_busy_state() {
  printf 'agy\t'; fm_backend_herdr_busy_state
}
fm_backend_herdr_agent_state() { printf live; }
SH
...
test_herdr_incomplete_shell_view_cannot_prove_absence() {
  ...
  [ "$out" = live ] || fail "an incomplete shell view must not prove agent absence, got '$out'"
  out=$(AGY_FIX_RESP="$dir/agent-get.json" AGY_FIX_PROC="$dir/process-info.json" AGY_FIX_LOG="$dir/calls.log" bash -c '
    . "$0/bin/backends/herdr.sh"
    fm_backend_herdr_pane_presence_state() { printf "present"; }
    fm_backend_herdr_cli() {
  ...
  pass "herdr exit detection: incomplete shell evidence retains conservative liveness and refuses closing"
}
...
SH
  chmod +x "$fakebin/agy"
  fm_fake_exit0 "$fakebin" treehouse gh-axi gh herdr
  printf '%s\n' "$fakebin"
}
...
run_agy_spawn() {
  ...
  HOME="$home" FM_ROOT_OVERRIDE='' FM_HOME="$home" \
    ...
    "$SPAWN" "$id" "$proj" --backend herdr --harness agy --mode no-mistakes --yolo off "$@" 2>&1
}
...
test_agy_dialog_race_waits_for_native_working() {
  local id rec out rc enters
  id="agy-race-z11-$$"
  rec=$(make_agy_spawn_case race "$id")
  read_agy_spawn_record "$rec"
  out=$(FM_FAKE_AGY_IGNORE_TRUST=1 FM_FAKE_AGY_RACE=1 run_agy_spawn "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" \
    "$FAKEBIN_DIR" "$id" --model gemini-3.8-flash-low)
  rc=$?
  expect_code 0 "$rc" "a delayed vendor dialog must be answered before native working is accepted: $out"
  [ "$(cat "$CASE_DIR/agy.state")" = busy ] || fail "spawn completed before the dialog answer worked"
  enters=$(count_enter_sends "$CASE_DIR/tmux-calls.log")
  [ "$enters" -eq 2 ] || fail "delayed dialog must receive exactly one answer after launch, got $enters Enter sends"
  pass "agy delayed dialog is answered before native working confirms readiness"
}
...
test_agy_unregistered_path_without_a_dialog_fails_the_spawn() {
  ...
  out=$(FM_FAKE_AGY_ASSUME_TRUSTED=1 run_agy_spawn "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" \
    "$FAKEBIN_DIR" "$id" --model gemini-3.8-flash-low) || rc=$?
  [ "$rc" -ne 0 ] || fail "a corrupt trust store must refuse even if a vendor would show no dialog"
  assert_contains "$out" "could not grant agy workspace trust" "trust failure did not name the cause"
  assert_not_contains "$out" "spawned $id" "unregistered workspace reported a successful spawn"
  [ ! -s "$CASE_DIR/launch.log" ] || fail "trust failure launched an agent"
  [ "$(cat "$store")" = '{not json' ] || fail "trust failure overwrote the broken store"
  pass "agy trust failure refuses before launching, independently of vendor readiness"
}
...
test_agy_spawn_arms_no_busy_wiring() {
  ...
  for sidecar in "$statedir/$id.agy-"*; do
    [ -e "$sidecar" ] || continue
    # Trust custody is retained for exact cleanup, independently of busy wiring.
    [ "$sidecar" = "$statedir/$id.agy-trust" ] && continue
    fail "agy spawn left an unexpected adapter sidecar behind: $sidecar"
  done
  pass "fm-spawn: agy arms no busy wiring and retains only trust custody"
}
