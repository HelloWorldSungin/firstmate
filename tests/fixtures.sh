...
#
# FM_FAKE_RESOLVED_CLAUDE_CONFIG_LOG is a second env-gated recording on the same
# send-keys payload: for a claude launch it writes the `.claude.json` the pane
# would actually open, taking the pinned CLAUDE_CONFIG_DIR from the launch
# command when the spawn forwarded one and otherwise FM_FAKE_DAEMON_CLAUDE_CONFIG_DIR
# or HOME. It exists because a launch command that merely CONTAINS the right
# directory does not prove the pane resolves to it ahead of an ambient daemon
# config. Both recordings stay inert when their variable is unset.
...
fm_test_run_spawn() {
  ...
  # Store-resolution fixtures may explicitly select another disposable home.
