...
test_housekeeping_captain_held_resurfaces_and_resets() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  FM_HOME="$dir" FM_STATE_OVERRIDE="$state" "$ROOT/bin/fm-afk-contract.sh" propose >/dev/null || fail "could not propose away posture"
  FM_HOME="$dir" FM_STATE_OVERRIDE="$state" "$ROOT/bin/fm-afk-contract.sh" confirm >/dev/null || fail "could not confirm away posture"
