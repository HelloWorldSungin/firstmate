...
run_spawn() {
  ...
  CLAUDE_CONFIG_DIR="${FM_TEST_CLAUDE_CONFIG_DIR:-}" \
    FM_FAKE_LAUNCH_LOG="$launchlog" FM_FAKE_TEXT_LINE_LOG="$launchlog.text-lines" FM_FAKE_PI_VERSION="${FM_TEST_PI_VERSION:-0.84.0}" \
...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  out=$(FM_TEST_SPAWN_USER_HOME="$runtime_home" \
...
test_relative_home_overrides_launch_with_absolute_cross_process_paths() {
  ...
  out=$(
    ...
    CDPATH="$CASE_DIR/cdpath" FM_ROOT_OVERRIDE='' FM_HOME=home \
      ...
      HOME="$HOME_DIR/user-home" "$SPAWN" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1
  )
  status=$?
...
test_home_defaults_preserve_absolute_or_resolve_relative_paths() {
  ...
  out=$(
    ...
    FM_ROOT_OVERRIDE='' FM_HOME=home \
      ...
      CLAUDE_CONFIG_DIR='' FM_FAKE_LAUNCH_LOG="$LAUNCH_LOG" \
      GROK_HOME=home/grok-home PATH="$FAKEBIN_DIR:$PATH" \
      HOME="$HOME_DIR/user-home" "$SPAWN" "$relative_id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1
  )
  status=$?
  ...
  out=$(
    FM_ROOT_OVERRIDE='' FM_HOME="$linked_home" \
      ...
      HOME="$HOME_DIR/user-home" "$SPAWN" "$absolute_id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1
  )
  status=$?
  ...
  for harness in pi pi-signed; do
    for mode in no-mistakes direct-PR; do
      ...
      fm_test_spawn_brief "$HOME_DIR" "$id" \
        "$(printf 'brief for %s\n<!-- firstmate-task-branch=fm/%s -->\nDelivery contract: mode=%s' "$id" "$id" "$mode")"
...
test_claude_forwards_firstmate_config_dir_when_set() {
  ...
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='$cfg' env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude" \
...
EOF
    ...
    assert_contains "$(cat "$LAUNCH_LOG.text-lines")" "export FM_TASK_ID=$id" \
      "design worker did not receive the primary-checkout isolation marker"
...
test_spawn_copies_continued_task_branch_from_brief_flag() {
  ...
  python3 - "$HOME_DIR/data/$id/brief.md" <<'PYFIX'
from pathlib import Path
import sys
brief = Path(sys.argv[1])
brief.write_text(brief.read_text().replace("{TASK}", "Continue the existing branch.")
                 .replace("{FIRSTMATE_SPEC}", "Preserve the continued branch identity."))
PYFIX
  runtime_home="$CASE_DIR/runtime-home"
  mkdir -p "$runtime_home/.claude"
...
SH
    ...
    assert_grep 'report to firstmate; do not adopt the supervisor identity' "$prompt" "$kind command lost the worker reporting boundary"
    assert_grep 'or address the captain' "$prompt" "$kind command lost the worker address exception"
