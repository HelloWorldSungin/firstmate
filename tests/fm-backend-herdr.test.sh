...
# These upstream fixtures use a sleeper as the process-tree root while the
# provider body describes a shell. Make that simulated identity explicit in
# the process table too, retaining every real descendant and its real name.
# The fork proof tests below supply complete native shell/TTY snapshots.
install_stale_shell_ps() {  # <directory> <process-info-json>
  local dir=$1 shell_pid
  shell_pid=$(printf '%s' "$2" | jq -r '.result.process_info.shell_pid // 0' 2>/dev/null) || shell_pid=0
  cat > "$dir/ps-overlay" <<'SH'
#!/usr/bin/env bash
ps -axo pid=,ppid=,stat=,comm=,tty= | awk -v root="$(cat "$(dirname "$0")/shell-pid")" '
{ pid[NR]=$1; parent[NR]=$2; state[NR]=$3; name[NR]=$4; tty[NR]=$5 }
END {
  owned[root]=1; changed=1
  while(changed) { changed=0; for(i=1;i<=NR;i++) if(parent[i] in owned && !(pid[i] in owned)) { owned[pid[i]]=1; changed=1 } }
  for(i=1;i<=NR;i++) {
    if(pid[i]==root) name[i]="zsh"
    if(pid[i] in owned) tty[i]="fmfixture"
    print pid[i],parent[i],state[i],name[i],tty[i]
  }
}'
SH
  printf '%s\n' "$shell_pid" > "$dir/shell-pid"
  chmod +x "$dir/ps-overlay"
}
...
stale_registration_case() {  # <dir-suffix> <agent_status> <process-info-body|-> [process-info-exit]
  ...
  install_stale_shell_ps "$dir" "$3"
  fb=$(make_herdr_fakebin "$dir")
  FM_HERDR_PS_BIN="$dir/ps-overlay" FM_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
    PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
...
settle_registration_case() {  # <dir-suffix> <polls> <process-info-body>...
  ...
  install_stale_shell_ps "$dir" "$1"
  fb=$(make_herdr_fakebin "$dir")
  FM_HERDR_PS_BIN="$dir/ps-overlay" PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
...
test_registered_agent_with_an_empty_foreground_over_a_real_shell_settles_via_descendant_walk() {
  ...
  kill "$shell_pid" 2>/dev/null || true
  [ "$out" = "live alive refused" ] \
    || fail "an incomplete foreground list must retain conservative liveness without licensing recovery, got '$out'"
...
test_busy_state_never_reports_a_shell_only_pane_busy() {
  ...
  install_stale_shell_ps "$dir" "$(cat "$resp/2.out")"
  fb=$(make_herdr_fakebin "$dir")
  out=$(FM_HERDR_PS_BIN="$dir/ps-overlay" PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
...
test_agent_state_inconclusive_process_read_stays_alive() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(run_agent_state "$fb" "$log" "$resp" "$dir/ps" 2)
  [ "$out" = unreadable ] || fail "an inconclusive process read must refuse recovery as unreadable, got '$out'"
  pass "fm_backend_herdr_agent_state: an inconclusive process read reports unreadable and refuses recovery"
}
...
# shape the queued-Enter conversion must refuse: the approval dialog owns the
...
# that to 'empty' would make fm-send.sh exit 0 on an undelivered steer
# (HelloWorldSungin/firstmate#84), so fm_backend_herdr_queued_enter_busy counts
# only native `working` as delivery-busy and a blocked pane keeps 'pending'.
test_send_text_submit_blocked_baseline_never_reaches_the_queued_enter_conversion() {
  local dir log resp fb out enter_count
  ...
  # 10: the retries-exhausted delivery-busy read, still blocked and therefore
  #     not working, so the queued-Enter conversion cannot fire.
  printf '{"result":{"agent":{"agent_status":"blocked"}}}\n' > "$resp/2.out"
...
test_send_text_submit_preexisting_working_pending_is_queued_enter() {
  ...
  # because the pre-Enter native status is already working, which is also why
  # this adapter skips the pre-Enter footer read entirely on that baseline.
  # 1: send-text  2: baseline working  3: enter  4: composer -> pending
  # 5: the retries-exhausted delivery-busy read -> working (queued).
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/2.out"
  printf '  \xe2\x9d\xaf hello captain\n' > "$resp/4.out"
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/5.out"
  fb=$(make_herdr_fakebin "$dir")
...
test_send_text_submit_preexisting_working_does_not_confirm_failed_enter() {
  ...
  # 1: send-text  2: baseline working (no pre-Enter footer read on that
  # baseline)  3: enter, which FAILS, so no delivery confirmation may run.
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/2.out"
  printf '1\n' > "$resp/3.exit"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
