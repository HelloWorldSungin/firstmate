...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # Force a MISSING diagnostic line so the bootstrap section is non-trivial. Use
  # no-mistakes, a firstmate-specific tool that is never on a standard system PATH,
  ...
  # The filtered base PATH below also excludes host node installations.
  rm -f "$fakebin/node"
...
EOF
  ...
  assert_contains "$out" "run /reload in this Pi session or restart plain pi so $root/.pi/extensions/fm-primary-turnend-guard.ts and $root/.pi/extensions/fm-primary-pi-watch.ts load their current code" "pi extension load diagnostic omits the turn-end guard extension or the reload activation"
  assert_contains "$out" "(/new, /resume, and /fork keep the extension code loaded earlier)" "pi extension load diagnostic presents session replacement as activation"
...
EOF
  ...
  assert_contains "$out" "run /reload in this Pi session or restart pi-signed so $root/.pi/extensions/fm-primary-turnend-guard.ts and $root/.pi/extensions/fm-primary-pi-watch.ts load their current code" \
