...
//
// A port of .pi/extensions/fm-primary-pi-watch.ts for the omp fork, using its
// arm, successor, retry, and replacement-handoff model with the fork's legacy
// away-daemon standby. The omp-specific differences are stated once here:
...
// The fork's legacy away-daemon ownership applies to this extension too.
// docs/watcher-continuity.md owns the handoff contract. Poll only while an
// extension generation is active; FM_OMP_AWAY_POLL_MS defaults to 2000ms.
const awayFlag = `${state}/.afk`;
const awayPollMs = positiveInteger("FM_OMP_AWAY_POLL_MS", 2000);
const awayStandbyMessage = "watcher: standby - away mode owns supervision while state/.afk exists; this extension resumes one cycle automatically on return";
