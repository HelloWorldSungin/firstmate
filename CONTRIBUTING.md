...
Every later head published after no-mistakes writes the PR body, including a merge commit or a fix commit produced by an active gate, must go through a fresh `git push no-mistakes`, never directly to `origin`, so the pipeline replaces the attestation with one bound to that head.
...
bin/fm-test-run.sh --check-coverage   # prove portable shards + serial + serial shards + Herdr + live opt-ins equal the full inventory
