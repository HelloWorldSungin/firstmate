...
Every later head published after no-mistakes writes the PR body, including a merge commit, a documentation-only follow-up, or any fix commit produced by an active gate, must go through a fresh `git push no-mistakes`, never directly to `origin`, so the pipeline replaces the attestation with one bound to that head.
