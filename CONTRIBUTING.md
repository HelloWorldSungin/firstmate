...
After creating any later commit, including a fix produced by an active gate, publish it only through a fresh `git push no-mistakes`, never directly to `origin`, so the pipeline replaces the PR body attestation with one bound to that new head.
