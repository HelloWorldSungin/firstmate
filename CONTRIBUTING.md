...
`bin/fm-brief.sh` emits that skill's load instruction automatically for ship tasks whose project checkout shares this repository's git object database, and gives ship and scout alike the role fact that this checkout's `AGENTS.md` describes firstmate rather than the worker; a crewmate picking up an older hand-written brief should still load the skill when the task touches shared tracked material.
