...
The deeper tmux agent-liveness probe reads `#{pane_current_command}` to distinguish a running harness process from a bare idle shell.
It classifies recognized Claude, Codex, OpenCode, and Grok process names as alive, common shells as dead, and every other value as unknown.
Unknown is intentionally safer than a false dead result because a false dead result could launch a duplicate agent.

Pi runs through a generic `node` process name and cannot be attributed confidently from the tmux foreground-process field.
A dead Pi secondmate is therefore reported as an inconclusive liveness result rather than auto-healed.
...
- Pi agent-process liveness is inconclusive and never triggers automatic replacement.
