...
The tmux submit core (shared `fm_tmux_submit_enter_core`) treats a busy pane + retries-exhausted + composer-still-pending as a queued Enter (opencode 1.18.4 accepts Enter mid-turn and queues it for after the turn), reported as `empty` so the daemon and `fm-send` do not re-send; an idle pane keeps the `pending` verdict as a genuine swallow. The herdr adapter applies the equivalent conversion without inheriting the tmux internals: after its own Enter retry budget it re-reads its ANSI-aware structural composer and, only on a proven pending composer, consults herdr's native agent-state under the same submit vocabulary the rest of that submit path uses, so a submit-active target reports `empty` and an idle one with visible unsubmitted text stays `pending`. A target that was already blocked at a permission prompt before the send is excluded from that conversion entirely, since a dialog that consumes the Enter is indistinguishable at the tail from a queued one.
