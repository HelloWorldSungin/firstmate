...
For a scout with no attributed run, the durable unresolved-decision inventory attestation reports done before backend liveness or status-log fallback; [`decision-hold-lifecycle.md`](decision-hold-lifecycle.md) owns that attestation and its follow-up reopening mechanism.
...
The herdr adapter applies the equivalent conversion without inheriting the tmux internals: after its own Enter retry budget it re-reads the shared composer classification and, only on a proven pending composer, consults herdr's native agent-state under the same submit vocabulary the rest of that submit path uses, so a submit-active target reports `empty` and an idle one with visible unsubmitted text stays `pending`.
A target that was already blocked at a permission prompt before the send is excluded from that conversion entirely, since a dialog that consumes the Enter is indistinguishable at the tail from a queued one.
...
Each tracked-output task's mode and `yolo` posture are firstmate's decision at intake and are passed explicitly to `bin/fm-brief.sh`, `bin/fm-spawn.sh`, and `bin/fm-promote.sh`, which refuse a ship or design task that does not carry them.
...
`data/projects.md` records each project's standing posture and optional `+yolo` flag as the captain's default and as context for that decision, including the conditional `no-mistakes-prod-only` policy; a ship or design spawn that drops below the registered rigor prints a deviation notice and continues.
