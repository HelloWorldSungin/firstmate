...
The forge read is the authority and the merge command's exit status is not: a non-zero `gh-axi pr merge` or `glab mr merge` whose follow-up read confirms the request merged is deliberately recorded as landed, reconciled out loud on both forges rather than exiting zero in silence, and returns zero, and a merge already observed as landed is never re-read.
An outcome the read does not confirm as landed records nothing instead: GitHub refuses non-zero, including when the outcome could not be read at all, while GitLab leaves the poll armed and fails the run only when the merge command failed too.
Both that verdict and the queued refusal are deliberate fork divergences recorded in [docs/fork-divergence.md](fork-divergence.md).
On GitLab an auto-merge-queued or unconfirmed request is reported without failing the run.
On GitHub an outcome that is neither merged nor queued is refused loudly and non-zero, naming the observed state, and a base branch that requires the merge queue is refused because this fork refuses deferred execution, naming a next step it can honour rather than a `--auto` retry it would itself reject.
Forwarded arguments are an allow-list rather than an unchanged passthrough, so `--auto` is refused by name before any merge is attempted and a deliberate-deferral request never reaches the forge.
