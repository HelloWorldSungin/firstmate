...
The forge read is the authority and the merge command's exit status is not: a non-zero `gh-axi pr merge` or `glab mr merge` whose follow-up read confirms the request merged is deliberately recorded as landed and returns zero, a merge already observed as landed is never re-read, and only a merge confirmed not to have landed returns non-zero.
