...
After a GitHub merge succeeds, `bin/fm-pr-merge.sh` closes at most one eligible `github.com` work item in the repository that item records; multiple items, unsupported forges or hosts, and bookkeeping failures warn without turning a completed merge into a failed, retryable merge.
