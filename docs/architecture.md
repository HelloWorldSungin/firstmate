...
A declared wait that is still unchanged at each recheck widens its own recheck window, up to `FM_PAUSE_RESURFACE_MAX_STREAK` doublings, so a long healthy wait stops costing a supervision turn at a fixed rate while still never rotting invisibly.
The widening is earned by one wait and dies with it: a crew that resumes, clears the pause, or declares a different wait is rechecked at the base window again, and no backoff ever widens the `FM_STALE_ESCALATE_SECS` wedge threshold.
The streak decides only how WIDE that window is; when the next recheck is due is anchored on the crew's status file mtime in the watcher and on when the hold began in the away-mode daemon, so a crew that keeps rewriting its paused reason is still rechecked on schedule while the captain is away.
...
So a delivered ordinary crew task whose last line stays `done: PR ...` bounds repeated alarms from new pane hashes to the declared-wait recheck cadence for the length of the captain's decision.
...
At the moment either supervisor would raise that wedge escalation - and only there, so a bounded pipeline read costs at most one call per window per pane rather than one per poll - it asks `bin/fm-run-progress.sh` whether the crew's validation run is actually moving, because "there is an active run" and "that run is progressing" are different facts and a review or test step routinely emits one opening line and then works silently for ten to eighteen minutes.
A run reported `progressing` holds the escalation and restarts the timer; a `stranded` run escalates and names the step that stopped, while a `none` verdict escalates unless the declared-wait fallback below applies.
One case answers to the crew instead of the run: when the pane's own last status line declares a wait (`paused:` or `captain-held`) that authoritative working state has outranked for tracking, a `none` verdict - no evidence either way - hands the pane back to that wait's own re-surface cadence in the always-on watcher rather than raising a wedge alarm, because run progress is evidence about the run and a declared wait is evidence about the worker, and an alarm needs a reason to fire rather than the absence of one.
A `stranded` run and a confidently dead agent are that reason and still escalate, so the fallback can only ever quiet the no-evidence case, and the away-mode daemon reaches the same answer by short-circuiting a declared pause before the recheck.
Consecutive holds for one pane are capped at `FM_RUN_PROGRESS_HOLD_MAX` (15, one hour of held silence at the default escalate window, matching the hour firstmate already allows a live-but-quiet endpoint through `FM_BUSY_TURN_MAX_SECS` and `FM_PAUSE_RESURFACE_SECS`), past which it escalates however healthy the run looks and carries the progress detail into the reason, because run progress is evidence about the RUN and not about the WORKER: a moving pipeline licenses a delay in alarming, never permanent silence for a worker that has hung while its pipeline keeps advancing.
The forced escalation resets the hold count but not the escalation count, so the cap behaves as a repeating check-in cadence while still counting toward `demand-deep-inspection`.
For a crew that declared no wait, that gate can only ever suppress on positive evidence: no run, a run parked at a gate, an unreadable status, and an endpoint whose agent is confidently dead all escalate exactly as they did before it existed, so a crew wedged with no run at all, a stranded step, and an orphaned step that left its run reporting `running` indefinitely all still alarm.
`bin/fm-run-progress.sh` owns the evidence (the pipeline's own `active_steps[].active_for` and `last_activity`) and the named `FM_RUN_STRANDED_SILENCE_SECS` bound that separates slow from stranded, while `bin/fm-classify-lib.sh`'s `crew_wedge_progress` is the single owner of the run-progress policy both supervisors apply.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn-boundary wake arrives; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, `demand-deep-inspection` marker, and run-progress hold and cap described above, for inspection only - never an automatic interrupt, signal, or restart.
...
The hold applies on that busy path deliberately: the busy bound exists to catch a hung foreground call, and a crew driving `no-mistakes axi run` in the foreground is exactly such a call, so whether its pane reads busy or stale is only an artifact of whether its harness backgrounded the pipeline call.
The one wedge-escalation caller the hold cannot reach is the parked-open-decision path, which is already guarded by a confidently dead agent and so is excluded by the dead-agent short-circuit.
...
The observation interval starts when that oldest epoch-sequence identity is first seen and resets whenever it changes, including a queue reprovisioned under the same task id; declared external-wait rows are excluded.
The semantic verdict from [`bin/fm-busy-lib.sh`](../bin/fm-busy-lib.sh) selects the no-progress threshold: proven idle uses `FM_SECONDMATE_WAKE_IDLE_STALL_SECS`, while unknown, dead, and unreadable verdicts use `FM_SECONDMATE_WAKE_STALL_SECS` because uncertainty is not evidence of idleness.
Proven busy suppresses the alarm only within `FM_BUSY_TURN_MAX_SECS`; after that bound it uses the unknown-state backstop, and any alarm requests inspection rather than interrupting the mate.
One keyed parent notification covers each no-progress episode, with receipts and queued-key deduplication surviving watcher and handling crashes; empty or advancing queues reset the episode.
The record-backed verdict path remains dormant for normally spawned local secondmates whose launch does not arm the parent-owned busy-state record, while native Herdr and Grok verdicts can establish busy without that record.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when every referenced task independently has positive evidence that its crew is still working: an attributed active no-mistakes step, its recent age-bounded degraded replay after a lookup failure, or an exact busy verdict from the semantic busy-state contract, all read through `bin/fm-crew-state.sh`.
...
A `kind=secondmate` task's status signal is the parent-directed reply stream and is never absorbed as provably working, however strong that crew's working evidence is; its bare turn-ended signal is absorbed only by the ordinary authoritative working proof because an active secondmate does not enter the staleness backbone that would resurface deferred pane-churn evidence.
That rule governs the no-verb absorb only, so a mate that declares its own wait still earns the bounded pause cadence below exactly as an ordinary crew does.
A crew that declares `paused:` for a known external wait is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
A crew that declared such a wait and then started a validation run is tracked by the wedge timer instead, but the declaration stays on record as that timer's no-evidence fallback, so declaring the wait can never cost a crew an alarm it would not otherwise get.
A declared `paused:` earns that cadence on its own, whether or not its agent is still alive: the generated brief tells a crew to append the wait and stop, which on every verified harness leaves the agent alive at its prompt, so requiring a dead agent would deny the cadence to exactly the crew that followed its instructions.
...
When that captain-relevant status includes a still-open keyed decision, the one-shot suppressor is keyed on a digest of the explicitly keyed open-decision set returned by `bin/fm-classify-lib.sh`'s `status_open_decisions --explicit-only` view rather than on the pane hash, because an idle harness pane repaints on its own and a hash-keyed suppressor re-surfaced an unchanged already-escalated waiting state on every repaint.
...
A crew parked on such an already-surfaced open decision whose backend confidently reports its agent dead still escalates through the same wedge timer.
...
A turn-ended-only queue row omits its historical latest-status annotation only when that status file exactly matches the same seen marker, while new or uncertain status bytes and direct status rows keep their annotations.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run to the task's own recorded branch while reconciling the run head with pipeline-owned commit behavior, keeps an advancing run-step authoritative even if the pane has closed, and orders a terminal verdict against later declared waits.
A plain working verdict is additionally cross-checked against the endpoint's agent liveness, because the shared no-mistakes daemon keeps a run advancing after the worker exits cleanly; a confidently dead or missing agent reports the actionable `abandoned` state (the run is moving with nobody driving it), an unreadable or ambiguous liveness read reports `unknown` rather than resolving either way, and the check runs only on a working verdict so secondmates and terminal states pay nothing for it.
The same mapping is applied when a degraded replay would re-emit `working`, so a record written before the worker died cannot keep reading as working for the rest of the degrade window.
That identity is the durable `branch=` written into a ship or design task's metadata when it is spawned, or into a scout's metadata when it is promoted, rather than whichever branch the pooled worktree currently has checked out.
A named worktree branch that disagrees with it is an attribution fault on its own, and a legacy record that never captured one makes any run found through its ambient branch unattributable as well.
Both report `unknown` under source `run-attribution` rather than attributing another task's run or falling back to the status log, while a legacy record with no run at all keeps the ordinary pane and status-log fallback.
The update transition is deliberate: locked bootstrap may add a missing branch only when a task's recorded canonical GitHub PR and recorded PR-head SHA match the forge's current head SHA, making the branch returned by that same response independent of ambient worktree placement.
...
A `paused:` or `captain-held` line recorded after a terminal done or failed verdict reports paused with its wait reason, except when `outcome=passed` with `ci` completed proves the forge merge and makes that wait stale; a declared wait recorded before a still-advancing run remains stale and the active run-step stays authoritative.
When no run is attributed, an exact busy verdict reports working before any degraded or fallback source.
For a scout with no attributed run, the durable captain-call inventory attestation reports done before backend liveness or status-log fallback; [`captain-hold-lifecycle.md`](captain-hold-lifecycle.md) owns that attestation and its follow-up reopening mechanism.
...
A positive daemon socket-failure status remains blocked even over an attributed run; other transport claims may be superseded by recent active-step evidence under the current-state reader's contract.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled or if work, process-event sources, registered custom checks, Relay polling, or queued wakes has an unhealthy model-aware supervision verdict; on main it also warns when queued wakes are waiting for main itself to drain.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for secondmate recovery, Zellij remains unverified, and Orca and cmux do not support secondmate spawns; [`model-verification.md`](model-verification.md) owns how teardown combines those backend capabilities with its no-turn evidence boundary.
...
The [`fm-spawn.sh` header](../bin/fm-spawn.sh) owns ship/design/scout worktree isolation and fresh-base refusal rules, including spawns from linked homes.
...
Ship, design, and scout dispatch can stay on the static crewmate harness resolved by `config/crew-harness`, or it can use local dispatch profiles in `config/crew-dispatch.json`.
...
When the file exists, `fm-spawn.sh` refuses ship, design, and scout launches without an explicit harness, so `config/crew-harness` is only automatic when no dispatch profile file is active.
Secondmate launches are exempt because they resolve the secondmate harness and any optional secondmate model or effort tokens instead.
Unsupported effort values are still recorded in task meta when passed to `fm-spawn.sh`, but the launch template omits any effort flag that the selected harness does not accept.
That keeps each selected launch compatible while preserving the requested profile for later audit; the `harness-adapters` skill owns the verified support matrix.
...
Each tracked-output task's mode and `yolo` merge posture are firstmate's decision at intake.
The mode is passed explicitly to `bin/fm-brief.sh`, and both values are passed explicitly to `bin/fm-spawn.sh` and `bin/fm-promote.sh`; each command refuses to guess the values it consumes, and refuses a ship or design task that does not carry them.
...
Project-scoped work-item linkage is resolved once at intake against the project's declared tracker and then carried explicitly through the brief and task metadata; the [configuration reference](configuration.md#project-issue-trackers) owns the registry schema, accepted references, credentials, enrichment, write-back to the tracker and the captain's board, and merge-bookkeeping limits.
The legacy `bin/fm-brief.sh --issue <number>` input remains only for a same-repository GitHub issue, while `--work-item <forge>:<url>` preserves the tracker identity needed by mirrored and cross-forge projects and requires the `--pr-target` that decides which of those references firstmate may write back to.
Write-back itself runs one lifecycle vocabulary through `bin/fm-work-item-milestone.sh` onto both surfaces firstmate keeps true, the work item's living status comment and the captain's project board, so the two cannot hold different opinions about a task; both are decoration that fails open and can never fail the work they describe.
Every per-host forge write crosses one owner, `bin/fm-forge-lib.sh`, whose header owns the write-operation allowlist, the minimum token scope per forge, and the adapter-shaped extension path a further forge follows.
The helper requires a full canonical URL and rejects malformed URLs or repo override flags before recording merge state.
A `https://github.com/<owner>/<repo>/pull/<n>` URL invokes `gh-axi pr merge <n> --repo <owner>/<repo>`, defaults to `--squash`, and preserves explicit merge-method flags, which on GitHub include `--rebase` because a GitHub rebase merge replays onto the base without rewriting the source branch.
...
The forge read is the authority and the merge command's exit status is not: a non-zero `gh-axi pr merge` or `glab mr merge` whose follow-up read confirms the request merged is deliberately recorded as landed, reconciled out loud on both forges rather than exiting zero in silence, and returns zero, and a merge already observed as landed - by the pre-merge target read as much as by any later one - is never re-read.
An outcome the read does not confirm as landed records nothing instead: on GitHub an outcome that could not be read at all refuses non-zero, while GitLab leaves the poll armed and fails the run only when the merge command failed too.
That verdict is a deliberate fork divergence recorded in [docs/fork-divergence.md](fork-divergence.md); the fork's earlier refusal of a merge-queue entry was retired by captain decision on 2026-09-03, so a queued request is again reported as `verified: queued` on exit zero.
Guarded merging on GitHub is limited to the repository's current default branch: a pull request targeting anything else is refused by name, naming both the target and the default, before any queue, auto-merge, or method handling, and both branches come from the same GraphQL query or, when `gh` is degraded, from gh-axi's own `api` passthrough; `HelloWorldSungin/firstmate#257` tracks extending the contract to GitLab.
That target check is a precondition rather than an outcome, so it is evaluated even when the pull request already reads merged: one merged into a non-default branch elsewhere is refused, saying so by name, instead of being recorded as this task's landed outcome.
It is settled before `pr=` is recorded and the merge poll armed, because `bin/fm-pr-poll.sh` reports a merged pull request with no notion of the branch it merged into, so a poll armed behind that refusal would record the very landing the refusal exists to keep off this task's ledger.
The permitted target is also pinned by state and not only by name: the default branch's tip commit is read when that contract is settled and re-read immediately before the merge call, and a tip that moved in between refuses naming both commits, so an immediate merge executes against the base this run judged; neither forge CLI exposes an expected-base parameter, so the residual sub-second window is declared beside the check rather than closed, and an already-landed merge skips it because it has no base left to judge.
...
Forwarded arguments are still an allow-list rather than an unchanged passthrough, so `--auto` is refused by name before any merge is attempted and a caller who takes that retry guidance is turned away by the allow-list rather than by the forge.
...
A learning that no longer earns a startup-memory slot but is still true is captured into the home's own brain before it is pruned, so pruning is not forgetting ([`gbrain-capture.md`](gbrain-capture.md)).
The internal [`stow` skill](../.agents/skills/stow/SKILL.md) owns tier markers, decay, cold archival, and captain-gated offload.
...
The stow pass never writes a tracked skill; its only direct skill writes are adding to an existing user-owned local destination, creating the one bounded bootstrap destination for a home with none, and creating a validated weight or fit split, with the internal `stow` skill above owning the exact contract.
A separately executed, captain-approved migration may move pinned conditional knowledge into a user-owned local skill excluded from the Firstmate clone.
