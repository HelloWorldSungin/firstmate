...
The script header owns the exact run-head ancestry rules, including the two cases the pipeline's own commits create: an actively executing run keeps attribution when its fix commits advance the tip past the sha it recorded, and a live run answered for this branch keeps it when the head it reports is not an object in the worktree at all because the pipeline is committing in a copy the worktree has never fetched.
A parked, terminal, rewritten, or diverged run does not, and neither does an unresolvable sha read out of the historical runs listing rather than answered for the current branch.
A run lookup that cannot complete is held distinct from a branch that has no run: a failed or timed-out lookup degrades to that crew's own last observed run-step under source `run-step-degraded`, bounded by age and refused once the crew's endpoint is gone, so a saturated validation daemon no longer turns a working crew into a wedge suspect without letting a permanently unreachable one hide a real wedge.
During no-mistakes' `ci` monitor phase, it also reads the ci step log tail because `axi status` reports both "still waiting on checks" and "checks green, waiting on merge" as `ci,running`.
The most recent recognized ci log marker wins, so checks-green monitoring reports done while a later re-arm, failed-check, or issue marker returns the crew to working.
Only when a completed lookup finds no matching run does it consult semantic busy state; exact busy reports working, exact idle permits fallback to a status-log event whose verb maps to a recognized run-state, and unknown or a dead pane stays unknown instead of trusting a stale log.
...
For same-repository GitHub issue work, `bin/fm-brief.sh --issue <number>` places one explicit issue identity in the ship brief, and `bin/fm-spawn.sh` validates and copies that identity into task metadata without inferring it from prose.
After the GitHub merge succeeds, `bin/fm-pr-merge.sh` verifies that recorded issue is closed and closes it with the merged PR URL when needed.
Issue bookkeeping failures remain loud but do not turn a completed merge into a failed, retryable merge; tasks without an explicit issue and non-GitHub providers retain their existing paths.
