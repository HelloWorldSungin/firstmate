...
Each tracked-output task's mode and `yolo` merge posture are firstmate's decision at intake and are passed explicitly to `bin/fm-brief.sh`, `bin/fm-spawn.sh`, and `bin/fm-promote.sh`, which refuse a ship or design task that does not carry them.
