...
That path recognises an already-merged request before pre-merge validation, and otherwise merges only after a final live read confirms it is open, mergeable, conflict-free, with blocking discussions resolved and a successful pipeline at the current head; it binds the merge to that verified head because recorded metadata is never authoritative after a rebase.
