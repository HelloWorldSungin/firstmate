...
A run reported `progressing` holds the escalation and restarts the timer; every other answer escalates, and a `stranded` run also names the step that stopped.
...
That gate can only ever suppress on positive evidence: no run, a run parked at a gate, an unreadable status, and an endpoint whose agent is confidently dead all escalate exactly as they did before it existed for a crew that declared no wait, so a crew wedged with no run at all, a stranded step, and an orphaned step that left its run reporting `running` indefinitely all still alarm.
`bin/fm-run-progress.sh` owns the evidence (the pipeline's own `active_steps[].active_for` and `last_activity`) and the named `FM_RUN_STRANDED_SILENCE_SECS` bound that separates slow from stranded, while `bin/fm-classify-lib.sh`'s `crew_wedge_progress` is the single owner of the policy both supervisors apply.
