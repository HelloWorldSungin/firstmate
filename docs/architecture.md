...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run to the task's own recorded branch while reconciling the run head with pipeline-owned commit behavior, then keeps that run-step authoritative even if the pane has closed.
...
A terminal pass whose own `ci` step was skipped never observed the forge at all - that step is what watches a pull request to merged or closed, and no-mistakes skips it on a provider it does not recognize - so the verdict reports the local pipeline passing instead of naming a merge, and stays a pure read of captured run output that never puts a forge call on a supervision poll.
A declared external wait standing over such a run is reported as that wait rather than done, because a finished local pipeline and a pull request nobody has acted on agree rather than contradict; a run that did observe the forge still reports done, and so does a crew that declared no wait.
