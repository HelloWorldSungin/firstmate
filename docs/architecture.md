...
A `https://github.com/<owner>/<repo>/pull/<n>` URL invokes `gh pr merge <n> --repo github.com/<owner>/<repo> --match-head-commit <inspected-head>`, defaults to `--squash`, and preserves explicit merge-method flags.
...
Both paths recognise an already-merged request before open-request validation and route it directly to outcome reporting without another mutation.
An open request is inspected from the task's own worktree, whose single `origin` must resolve to the pull request's host and path, because that origin is where the current default branch, its tip, and the pull request head are read and fetched from; a missing worktree, a mismatched origin, or an origin that cannot supply those refs refuses the merge and asks the operator to restore worktree and origin access.
For an open request, the actual target must be the repository's current default branch before any other target-dependent check runs; a mismatch is refused with retarget, update, and revalidation guidance.
At the merge boundary, live forge state binds the target, target OID, and head OID to the fetched repository objects, while GitLab additionally requires a mergeable, conflict-free request with blocking discussions resolved and a successful pipeline at the current head.
Deferred execution is refused: GitHub must positively show no merge queue and no active auto-merge, and GitLab passes `--auto-merge=false` while refusing automatic rebase when the project method can apply it to a source branch that is behind its target.
An unreadable deferral, identity, merge-method, ancestry, or automatic-rebase signal fails closed; a pre-19.4 GitLab response without `automatic_rebase_enabled` is permitted only when `merge_method=merge` or the target is positively known to be an ancestor of the source.
Forwarded arguments are an allow-list of immediate merge-method selectors, commit-message inputs, and post-execution branch cleanup; GitLab rebase selectors are excluded because they can change the source branch before a SHA-bound merge fails.
...
That read, not the merge command's exit status, is the authority: a non-zero `gh pr merge` or `glab mr merge` whose follow-up read confirms the request merged is deliberately recorded as landed and returns zero, because a transport or response failure after the merge landed is not a failed merge, and only a merge confirmed not to have landed returns non-zero.
A confirmed-unlanded result names the cause it actually observed, reporting a non-zero merge command as a forge rejection rather than as a deferral.
