...
Project-scoped work-item linkage is resolved once at intake against the project's declared tracker and then carried explicitly through the brief and task metadata; the [configuration reference](configuration.md#project-issue-trackers) owns the registry schema, accepted references, credentials, enrichment, and merge-bookkeeping limits.
The legacy `bin/fm-brief.sh --issue <number>` input remains only for a same-repository GitHub issue, while `--work-item <forge>:<url>` preserves the tracker identity needed by mirrored and cross-forge projects.
