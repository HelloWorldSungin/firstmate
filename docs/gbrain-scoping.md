...
`bin/fm-recall.sh`'s `--help` owns the answer protocol that search document carries: rows are nearest indexed pages rather than answers, a miss is absence of a match rather than absence of the queried thing, each corpus's own top `rerank_score` below that corpus's own effective `search.autocut_min_top` is named as no confident match, with the corpora that cleared named on the answer, a corpus whose brain turns `search.autocut` off in its database plane is left unjudged rather than measured against a floor its search never applied, each local page carries its capture date and live-source state, and a live source that has moved on is marked so rather than presented as current.
