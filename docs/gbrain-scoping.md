...
`bin/fm-recall.sh`'s `--help` owns the answer protocol that search document carries: rows are nearest indexed pages rather than answers, a miss is absence of a match rather than absence of the queried thing, confidence is judged per corpus against the weak-top floor GBrain itself applies so an answer can report no confident match and name the corpora that cleared, each local page carries its capture date and live-source state, and a live source that has moved on is marked so rather than presented as current.
