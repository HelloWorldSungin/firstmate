...
`tests/fm-quota-array-dispatch-live-e2e.test.sh` drives the public Pi skill-loading interface against one fake schema-5 snapshot per case, served as quota-axi's default TOON, plus a fake `bin/fm-quota-sidecar.sh` in the lab project.
It covers TOON-first `spendPriority` ranking among candidates that pass eligibility, reasoning-class, and runway-feasibility gates, explicit accounting for unmeasurable runway, the strongest-reasoning constraint, the runway feasibility floor over a higher `spendPriority`, a provider family absent from the snapshot that must stay eligible on its catalog-resolved surface, and `quota-axi` staying authoritative where a disagreeing sidecar overlaps it.
...
The sidecar is a separate additive reader rather than a `--json` call: it supplies eligibility evidence only for providers `quota-axi` does not cover and never ranks, so the suite asserts it cannot influence selection order.
`tests/fm-quota-sidecar.test.sh` owns the sidecar reader's own executable interface: fresh, stale, producer-error, unmounted-directory, missing-provider, malformed-document, clock-drift, and aggregate behavior, the rejection of an unusable freshness override, and the projection that never renders an unapproved producer field.
