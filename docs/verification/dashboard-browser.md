...
Landing that half does not retire the `exit_nonzero` form: the collector failure behind it was reproduced on this same home on 2026-08-08 against a healthy 37 MB store, because the hardened unit's sandbox leaves SQLite no writable scratch path for a read-only query's temp file.
