...
Five pointers, one per outcome the resolver can reach:
...
broken     /tmp/pointer-cases.md:4  https://github.com/HelloWorldSungin/firstmate/blob/main/docs/this-file-does-not-exist.md  ref-or-path-not-found
           the repository is visible, but docs/this-file-does-not-exist.md at ref main is not in it
...
fm-pointer-check: checked=5 ok=1 broken=3 unverified=1 skipped=0 files=1 credential=authenticated
...
fm-pointer-check: checked=5 ok=0 broken=0 unverified=5 skipped=0 files=1 credential=none
fm-pointer-check: 'unverified' is not a pass and not a failure - those pointers were not resolved either way.
$ echo $?
0
```
...
It exits 0, because failing to reach a target is not evidence against the target.
...
CI runs the check with this flag, so a lost credential surfaces as its own failure rather than as a green run over five unresolved pointers.
...
ok         .../Trading-Signal-AI/Operations/Deployment-Guide.md:189  https://github.com/HelloWorldSungin/ArkNode-AI/blob/master/docs/design/2026-08-10-ct110-sealed-corpus.md  path-resolved [scope=file]
...
fm-pointer-check: checked=23 ok=9 broken=0 unverified=0 skipped=14 files=86 credential=authenticated
$ echo $?
0
```

The 14 skipped pointers are hosts with no resolver in this check (`img.shields.io`, `discord.gg`, `x.com`, `skills.sh`, `herdr.dev`, `cmux.com`, `gitlab.com`, `huggingface.co`, `w3.org`) and are reported as skipped rather than counted as verified.
...
It asserts the verdict split rather than the presence of output: that an authenticated run separates the three outcomes, that a credential-less run produces neither an `ok` nor a `broken` column, that `--require-credential` exits 2, that a throttled lookup is unverified, and that fenced, inline, and templated URLs stay out of the pointer surface.
...
ok - fenced, inline, and templated URLs are excluded from the pointer surface
...
FM_TEST_SUMMARY total=1 failed=0 skipped_gate=0 duration_ms=327
```

The live half of this page - real credential, real private repository, real nonexistent owner - is what the stub cannot prove, and is the reason these commands are recorded rather than assumed.
