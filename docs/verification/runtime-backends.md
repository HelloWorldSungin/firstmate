...
The stale-registration cross-check was verified on 2026-08-15 against Herdr 0.8.0 with the same guard:
...
In this guard, `herdr pane report-agent` first creates the stale-registration state over a plain idle shell, then a real foreground process makes the paired live case fail the agent-free proof.
