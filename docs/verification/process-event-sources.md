...
| missing artifact classified, never auto-retired | the `ENOENT` a deleted artifact produces - carrying the `realpath` failure lavish-axi reports for it - classifies as `artifact-missing` rather than `unknown` and is deliberately not terminal, so a source whose artifact was deleted stays armed and keeps producing one result per poll until it is explicitly retired; an unrelated `ENOENT` without that `realpath` signal stays `unknown`, because classification never sees the artifact path and a broader match would tell a handler to retire a live review whose artifact is fine |
