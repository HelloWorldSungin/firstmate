...
Agreement and a home with no GBrain installed are both silent, `FM_BOOTSTRAP_PIN_TIMEOUT` owns that bound (default 10 seconds), and [`gbrain.md`](gbrain.md) owns the recorded pin, its upgrade policy, and the verdicts that line carries.
...
FM_BOOTSTRAP_PIN_TIMEOUT=10          # seconds allowed for bootstrap's read-only GBrain recorded-pin comparison, which runs only where this host resolves both a gbrain executable and a docs/gbrain.md; blank, non-numeric, or zero falls back to 10
