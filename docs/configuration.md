...
A run that stops early records the last registry entry it reached in `state/.board-sweep-cursor` and the next run starts after it, wrapping, so a budget too small to cover the whole fleet at once still covers all of it across a bounded number of runs rather than reconciling the same first projects forever.
...
A truncated run also names the entries it did not reach, because "drift may remain" and "these boards were not looked at" are different facts.
