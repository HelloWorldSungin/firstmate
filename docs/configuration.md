...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn-boundary wake arrives, before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart. bin/fm-supervision-lib.sh owns the window, and bin/fm-fleet-snapshot.sh publishes it as supervision.watcher.quiet_allowance_seconds so the dashboard's Task activity signal judges quiet against this same tolerance instead of a constant of its own, which means raising it also delays that signal by the same amount
