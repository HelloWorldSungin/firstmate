...
FM_PAUSE_RESURFACE_MAX_STREAK=3    # how many times an unchanged declared wait may double its own recheck window before the cadence stops widening; 0 restores a fixed FM_PAUSE_RESURFACE_SECS cadence, and the streak restarts whenever the wait itself changes. Clamped internally to 12 doublings and a one-day window, so a misconfigured value cannot overflow into a permanent re-surface
