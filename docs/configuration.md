...
The whole-operation budget says so wherever it truncates, on a read as much as on a write: reading a board and its tracker is where nearly all of a bounded sweep's time goes, so a budget that runs out there is reported as the truncation it is rather than as one more board the run could not reach.
