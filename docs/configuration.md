...
A page shorter than the one it asked for never ends that lookup, because a forge is free to clamp a list to its own maximum, and a walk that stopped there would miss the living comment and post a second one; when the page cap is reached without an answer, nothing is written and the uncertainty is reported.
