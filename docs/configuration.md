...
FM_BOOTSTRAP_USAGE_TIMEOUT=120       # seconds allowed for bootstrap's best-effort token-usage refresh, which runs only when data/usage.db exists; blank or non-numeric falls back to 120
FM_TEARDOWN_USAGE_TIMEOUT=60         # the same bound for the refresh task teardown runs before archiving a task
