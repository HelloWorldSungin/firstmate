...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before a declared external wait or verified captain-held transfer re-surfaces for a recheck in the watcher, including a live busy pane past FM_BUSY_TURN_MAX_SECS, and before an idle declared external wait re-surfaces in the away-mode daemon; the away-mode clock runs from when the hold began and is never restarted by the crew rewriting its paused reason
