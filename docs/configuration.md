...
FM_GBRAIN_MAINTENANCE_STATE=   # optional dashboard-visible maintenance mode: ready | upgrading | reindexing; see "Dashboard GBrain panel"
FM_GBRAIN_HEALTH_TIMEOUT=15   # seconds allowed for one fm-gbrain-health.sh read; the dashboard passes it to the capture-status and probe slices
