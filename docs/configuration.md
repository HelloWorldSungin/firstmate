...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, per-task completion manifests, per-task work-item references, explicitly installed content-addressed extension packages under `data/extensions/packages/`, and the token-usage store described in [usage-accounting.md](usage-accounting.md).
...
The tracked `.no-mistakes.yaml` keeps `test.evidence.store_in_repo: false`, so validation evidence stays local, and pins `commands.lint` to the repository's `bin/fm-lint.sh` owner.
The worktree's `.no-mistakes/` stays gitignored, and CI rejects tracked entries under that path.
...
The seed command itself does not create the JIT destination; during agent-led local provisioning, the [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md#charter-and-seed) then ensures that exact home has its first user-owned local destination.
...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.46.0 or newer, compatible gh-axi, compatible chrome-devtools-axi, compatible lavish-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
...
It arms the periodic captured-knowledge sweep on the same pass, which refreshes a page whose durable source changed and audits captured against served; [`gbrain-capture.md`](gbrain-capture.md) owns that sweep, its `FM_GBRAIN_CAPTURE_SWEEP_INTERVAL` clock, and its `GBRAIN_CAPTURE:` verdicts.
Agreement and a home with no GBrain installed are both silent, that script alone decides whether this host has a gbrain to compare against, `FM_BOOTSTRAP_PIN_TIMEOUT` owns that bound (default 10 seconds), [`gbrain.md`](gbrain.md) owns the recorded pin and its upgrade policy, and the script header owns the verdicts that line carries.
...
The same deferred network stage performs guarded tracked-file sync for recorded live homes and propagates declared inherited local material into each validated live home under that sequencing contract.
...
FM_SNAPSHOT_CACHE_READ_ONLY=0       # 1 reads existing cached ledgers without creating or writing the cache; the read-only dashboard forces 1
...
FM_BOOTSTRAP_CAPTURE_TIMEOUT=120     # seconds allowed for bootstrap's home-local captured-knowledge sweep, which refreshes stale pages and audits stored against served; blank, non-numeric, or zero falls back to 120. The sweep's own interval and audit ceiling belong to docs/gbrain-capture.md
