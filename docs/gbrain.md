...
[`bin/fm-gbrain.sh`](../bin/fm-gbrain.sh) `check` fails a `serving-credential` row when a home serves its brain and Firstmate's own configuration makes hosted synthesis reachable on it, so that declared configuration exits non-zero instead of passing unnoticed.
...
Past that gate the verdict reads Firstmate's declared surfaces alone: `think.secret` and `think.base_url` in this home's `config/gbrain.json`, and the `config/gbrain-secrets/<name>` credential plane that `think.secret` names.
Either route counts as hosted synthesis reachable - a named credential that is present and readable, or a `think.base_url` that leaves this host - because the credential a served `think` spends can be injected at runtime from the fleet-wide file in [MiniMax credential contract and privacy boundary](#minimax-credential-contract-and-privacy-boundary) rather than from any home's credential plane.
What the verdict deliberately does not read is GBrain's own runtime configuration - the `models.think` and `provider_base_urls.*` values that [Initialize and configure retrieval](#initialize-and-configure-retrieval) sets under `GBRAIN_HOME` - or the fleet-wide `/home/sungin/.pi/agent/auth.json` key that a runtime injection would supply.
A serving home configured only that way is reported clean, so the clean `serving-credential` row states its own reach in place of a bare pass: it names the surfaces it read and says the runtime `models.think` and a runtime-injected key are not inspected, because a pass here means "no violation in what this check reads" and never proof that hosted synthesis is unreachable; the rule above still binds that home.
The captain chose that split deliberately rather than widening detection here, and [issue 107](https://github.com/HelloWorldSungin/firstmate/issues/107) keeps the runtime-plane remainder open.
