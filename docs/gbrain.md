...
A serving home configured only that way is reported clean, so read a clean verdict as "no violation in the Firstmate surfaces this check reads" rather than as proof that hosted synthesis is unreachable; the rule above still binds that home.
