...
Past that gate the verdict inspects Firstmate's declared surfaces (`think.secret` and `think.base_url` in `config/gbrain.json`, and `config/gbrain-secrets/<name>`), GBrain's own runtime configuration (`models.think` and `provider_base_urls.*` under `GBRAIN_HOME`), and the fleet-wide runtime credential store (`/home/sungin/.pi/agent/auth.json`).
Hosted synthesis is reachable if a credential is held in the declared or runtime credential store, or if `think` points at a base URL that leaves this host.
