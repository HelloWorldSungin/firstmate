...
When `bin/fm-scout.sh` exists in the home the message first defers to the `AGENTS.md` intake classification, then routes work already classified as a scout there and authorized ship or design work to `bin/fm-brief.sh` then `bin/fm-spawn.sh`.
