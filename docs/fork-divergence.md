...
Bootstrap calls the GBrain serving and pin checks and emits the `GBRAIN_SERVING_CREDENTIAL:` and `GBRAIN_PIN:` diagnostics in [`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh) with their handling contract in the [`bootstrap-diagnostics` skill](../.agents/skills/bootstrap-diagnostics/SKILL.md), and session start composes that serving-credential state into its detect-only digest in [`bin/fm-session-start.sh`](../bin/fm-session-start.sh).
...
The shared test routing and documentation catalogs carry the rest of the surface: [`bin/fm-test-run.sh`](../bin/fm-test-run.sh), `tests/lib.sh`, `tests/fm-bootstrap.test.sh`, `tests/fm-brief.test.sh`, and `tests/fm-remote-secondmate-lifecycle-e2e.test.sh`, plus `docs/architecture.md`, `docs/configuration.md`, `docs/documentation-audiences.json`, and `docs/scripts.md`.
...
The fork carries a read-only fleet dashboard - the server and routed destinations behind [`bin/fm-dashboard-server.mjs`](../bin/fm-dashboard-server.mjs) and [`assets/dashboard/`](../assets/dashboard), the agent-event store and emitter, the durable completion manifests, and token-usage accounting - and upstream has no equivalent of any of it.
...
Teardown refreshes a task's usage sessions and publishes its durable history through the fork-owned manifest in [`bin/fm-teardown.sh`](../bin/fm-teardown.sh), and [`bin/fm-fleet-snapshot.sh`](../bin/fm-fleet-snapshot.sh) publishes the versioned snapshot contract the dashboard renders, including the quiet window owned in [`bin/fm-supervision-lib.sh`](../bin/fm-supervision-lib.sh) and consumed by [`bin/fm-watch.sh`](../bin/fm-watch.sh) so the dashboard's activity signal judges quiet on supervision's own tolerance.
[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) carries the Node 22 floor the usage collector requires and routes the dashboard browser suite, and the shared test routing and documentation catalogs named in the GBrain entry above carry this subsystem too.
...
A merge must also keep the dashboard off the fleet's critical path as [`dashboard.md`](dashboard.md) owns: the dashboard writes no fleet state, its one write is the event store it owns outside the operational home, and stopping it has no effect on Firstmate supervision.
Five files bridge this entry and the GBrain entry above - `assets/dashboard/gbrain.js`, `bin/fm-dashboard-server.mjs`, `bin/fm-gbrain-health.sh`, and the `tests/fm-dashboard-gbrain.test.sh` and `tests/fm-dashboard-gbrain-ui.test.sh` suites - because the dashboard's Knowledge view and health strip read the home's brain read-only rather than duplicating it.
