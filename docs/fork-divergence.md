...
The pinned shared `require-no-mistakes` action is still taken unchanged from `kunchenguid/firstmate#3027`, and a step above it runs the fork-local [`.github/scripts/nm-required-refresh-event-body.py`](../.github/scripts/nm-required-refresh-event-body.py), which has no upstream counterpart, to load the live pull-request body into the event payload so a synchronize webhook snapshot (and a GitHub rerun of that snapshot) is not judged in place of the body the pipeline wrote afterwards.
