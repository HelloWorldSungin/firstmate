...
Design teardown calls the same `verify` subcommand before removing the ADR task state, so an interactive design cannot bypass the shared captain-call inventory gate.
