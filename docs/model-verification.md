...
Before that allowance reaches any destructive cleanup, teardown consults the recorded backend's strongest available liveness evidence, terminates every endpoint not authoritatively reported live, and only then recomputes the terminal model-routing verdict and complete on-disk proof against the settled state.
...
A recovery-grade `dead` or `missing` result permits endpoint termination and the on-disk proof to decide, while `ambiguous`, `unreadable`, or `unverified` liveness is stated plainly and does not become either a safety proof or a permanent block.
The terminate-then-recompute-then-remove ordering is required because a clean snapshot taken while an unknown worker can still write could become stale before record removal and discard a late first turn.
