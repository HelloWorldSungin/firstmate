...
Before that allowance reaches any destructive cleanup, teardown consults the recorded backend's strongest available liveness evidence, then recomputes the terminal model-routing verdict and complete on-disk proof at the shared teardown boundary.
...
A recovery-grade `dead` or `missing` result permits the on-disk proof to decide, while `ambiguous`, `unreadable`, or `unverified` liveness is stated plainly and does not become either a safety proof or a permanent block.
