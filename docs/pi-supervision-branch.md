...
On Pi, `/afk` confirms the away-posture record (`state/.afk-contract`, owned by `bin/fm-afk-contract.sh`) before launching the away daemon.
While `state/.afk` exists, the branch and primary watcher extension stand by under [`watcher-continuity.md`](watcher-continuity.md), so only the daemon owns the supervision cycle.
After away mode clears, the extension resumes one cycle and ordinary branch routing returns.
