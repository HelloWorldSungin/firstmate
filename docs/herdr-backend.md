...
Firstmate requires Herdr protocol 14 or newer; versions 0.7.1, 0.7.3, and 0.7.4 are verified, with protocol-16 features enabled only when available.
...
Firstmate atomically publishes a three-field journal containing a random 128-bit base64url token before asking Herdr to create anything.
The token is visible in the workspace title because Herdr exposes no verified hidden persistent field, but neither token nor title authorizes send, capture, adoption, cleanup, or recovery.
...
A live or unknown recorded endpoint refuses duplicate launch.
A confirmed dead or agent-free endpoint may respawn flat while the old projected workspace remains untouched.
Zero, renamed, or duplicate token matches never authorize adoption or deletion; any live or unknown duplicate refuses launch.

Operational compromises:

- Grouping is best-effort within one healthy Herdr server lifetime.
- Existing layouts are not force-renamed or rearranged.
- Restarted projections can return as agent-free husks and remain for manual cleanup while the task respawns flat.
...
`tests/fm-backend-herdr-presentation-e2e.test.sh` covers multi-home ordering, concurrency, lock contention, legacy coexistence, focus preservation, restart husks, ambiguous tokens, and exact-pane cleanup through the guarded lab path.
...
Dead and no-agent map to dead, a registered agent maps to alive, and unreadable state remains unknown.
