...
A successful atomic prompt proves that Herdr queued input to the detected foreground agent process, but Herdr does not atomically assert that a cached non-blocked state still owns the UI at the write boundary, so cursor and agy report that accepted write as `unverifiable` with status 3.
