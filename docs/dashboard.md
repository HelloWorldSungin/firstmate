...
The store has writers - teardown refreshes it on every archive and bootstrap on every locked session start - and a read that overlaps one of them fails rather than reads empty.
A future migration to WAL journal mode would make the overlap read-only-unfriendly: the dashboard's read-only open needs write access to data/ for the wal-index, and the read would fail the same way the writer-hold case does.
The collector can also fail because the host sandbox leaves SQLite no writable scratch path for the temp file a read-only open needs; the generated unit pairs `ProtectSystem=strict` with `ProtectHome=read-only` and adds `PrivateTmp=yes` for exactly that reason, and a unit that loses the directive makes every read exit `disk I/O error` while the store itself is healthy.
[`docs/verification/dashboard-service-unit.md`](verification/dashboard-service-unit.md) pins the reproduced combination, the working fix, and the unit directive that keeps it in place.
