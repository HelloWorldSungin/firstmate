...
The collector can also fail because the host sandbox leaves SQLite no writable scratch path for the temp file a read-only open needs; the generated unit pairs `ProtectSystem=strict` with `ProtectHome=read-only`, which between them refuse `/tmp`, `/var/tmp`, and `$HOME`, so it grants a `RuntimeDirectory=` scratch path and points `TMPDIR` and `SQLITE_TMPDIR` at it, and a unit that loses those directives makes every read exit `disk I/O error` while the store itself is healthy.
`PrivateTmp=yes` supplies the same scratch path and is deliberately not used: it replaces the shared `/tmp` with a private tmpfs, which hides the fleet's tmux server socket at `/tmp/tmux-$UID` from the snapshot this service runs and reports every live task as `endpoint absent`.
[`docs/verification/dashboard-service-unit.md`](verification/dashboard-service-unit.md) pins the reproduced combination, the working fix, and the reproduction that rules `PrivateTmp=yes` out.
