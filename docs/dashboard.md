...
A collector that failed, or whose output this dashboard does not recognize, with no last good read to fall back on, leaves those cells reading `unavailable` too but is a fault someone can fix, so History carries a disclosure above the roll-up naming the failure and the reason the read gave.
That disclosure is the one thing that separates a breakage from a home where collecting nothing is correct, which is why it fires on no other state - including a failure the retained read below already covers, which states the same failure once rather than twice.
...
Results come back in a closed vocabulary - source, slug, title, score, excerpt, stale, captured_at, source_state, source_kind, source_id, source_updated_at, plus the search's answer framing - with anything outside it dropped rather than passed through, and the Knowledge view in [`assets/dashboard/app.js`](../assets/dashboard/app.js) builds every result node with `createElement` and `textContent`, so nothing in a stored brain document can become markup or restructure the page.
The answer framing is the same contract [`bin/fm-recall.sh`](../bin/fm-recall.sh) owns: rows are nearest indexed pages rather than matches, an empty list is no indexed match rather than a negative about the world, and a page whose live source has moved on is marked stale with its capture date.
`source_state` is that wrapper's page vocabulary - `current`, `drifted`, `uncompared`, `missing`, `snapshot`, `unknown` - and the endpoint passes each row's `stale` through as the wrapper decided it rather than re-deriving it from those states, so which of them count as stale has one owner.
`uncompared` means the page could not be checked against its live source at all; it is shown as itself rather than folded into `current`, because a page presented as current without a comparison behind it is the failure this whole surface exists to prevent.
