...
A collector that failed with no last good read to fall back on, or one whose output this dashboard does not recognize, leaves those cells reading `unavailable` too but is a fault someone can fix, so History carries a disclosure above the roll-up naming the failure and the reason the read gave.
