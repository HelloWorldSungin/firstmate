...
The unit therefore needs no scratch directive, which is what keeps `PrivateTmp=yes` out of it: that directive would replace the shared `/tmp` with a private tmpfs, hiding the fleet's tmux server socket at `/tmp/tmux-$UID` from the snapshot this service runs and reporting every live task as `endpoint absent`.
