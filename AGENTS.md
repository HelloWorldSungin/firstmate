...
2. **Bootstrap** - detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, documentation-vault drift, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the eight bootstrap MUTATING sweeps - non-executing legacy PR-check migration, legacy endpoint-binding migration, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, X-mode artifact writes, and the bounded token-usage store refresh - run only when this session actually holds the lock from step 1.
