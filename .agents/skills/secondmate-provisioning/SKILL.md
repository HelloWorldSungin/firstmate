...
If that path resolves to the crew-only cursor or agy adapter, configure a different concrete secondmate harness as required by `harness-adapters`; an explicit override cannot bypass that gate.
