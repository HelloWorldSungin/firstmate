...
**One-time rollout note:** the previous release executing this update may not restart all eligible live second mates.
After it completes, use `bin/fm-secondmate-restart.sh <fm-id>...` only for live homes proven successfully updated or already current at the exact intended target commit, excluding every home the update pass skipped.
The restart helper's runtime capability check does not prove version convergence; later updates follow the normal flow below.
...
5. **Report upstream drift without integrating it.**
...
6. **Report to the captain in plain outcomes.**
   ...
   Say plainly when a mate received a re-read message rather than a confirmed restart, and why.
