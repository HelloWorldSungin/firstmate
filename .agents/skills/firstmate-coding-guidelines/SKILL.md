...
Resolving the canonical `firstmate` name to a structurally matching home follows [`docs/configuration.md`](../../../docs/configuration.md) rather than registry prose or a secondmate marker, so read the scaffold and hand-write the instruction only when it is absent.
...
- Close a suite with the sibling trailing marker `printf '\nall <subject> tests passed\n'`, so a run that died partway through is visible as a missing final line instead of a quiet short pass.
  ...
  In suites that delegate teardown to `cleanup_all`, make `cleanup_all` propagate every teardown verdict without swallowing diagnostics, place the marker only after a successful cleanup, and make cleanup idempotent so an early test failure does not produce a false cleanup failure.
  The marker then proves the suite script reached its final statement and that teardown succeeded.
