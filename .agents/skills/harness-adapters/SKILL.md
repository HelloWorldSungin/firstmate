...
Send delivery uses that operation for a live native cursor or agy identity, with the successful identity-matched response serving as the native input-acceptance boundary.
...
An untyped transport failure or malformed success response reports `verdict=unverifiable` with status 3 and leaves scout completion reopened because acceptance is unknown.
