...
description: Agent-only reference for firstmate harness operations. Use before spawning or recovering a crewmate or secondmate, handling a trust dialog, sending a harness-specific skill invocation, interrupting or exiting an agent, resuming an exited agent, or verifying a new harness adapter. Contains verified facts for claude, codex, opencode, pi, grok, kimi, and the crew-only herdr-only cursor and agy adapters.
...
Optional dispatch profiles in `config/crew-dispatch.json` can override that static default for one crewmate, scout, or design dispatch by selecting concrete harness, model, and effort axes at intake.
...
`bin/fm-spawn.sh` uses `crew` mode for a crewmate, scout, or design launch and `secondmate` mode for a `--secondmate` launch, re-resolving on every spawn so the split is durable across respawns; an explicit per-spawn harness arg overrides either resolution but never the cursor/agy kind and backend gates.
...
| cursor | `--model <parameterized model string>` | none as a separate axis | Crew-only and herdr-only, verified on cursor-agent 2026.07.20. Effort is encoded inside the model string, such as `composer-2.5[effort=high]`, which `--model` accepts verbatim. |
| agy | `--model <model>` | `--effort <low\|medium\|high>` | Crew-only and herdr-only, verified on agy 1.1.5. `xhigh` and `max` are omitted. |
...
This is separate from the per-task crewmate turn-end hook above (that one lives in a task's own `.claude/settings.local.json` and just `touch`es a marker file, except for a `kind=design` task, whose Stop hook runs `bin/fm-design-context.sh` and signals the same marker unconditionally).
