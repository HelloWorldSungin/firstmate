...
Send delivery uses that operation for a live native cursor or agy identity and confirms acceptance only when that native session's transcript appends the exact user input.
The Herdr prompt response alone proves only that terminal input was queued.
...
An untyped transport failure, missing receipt, or malformed success response reports `verdict=unverifiable` with status 3 and leaves scout completion reopened because acceptance is unknown.
