...
Documented residual: an absolute-path invocation, a raw command that first resets PATH to drop the guard dir, or a raw command that removes the predictable guard dir with `rm -rf` before invoking the binary bypasses a PATH shim.
The raw command is firstmate-authored rather than attacker-supplied, so anything able to remove the guard already has full same-user execution and can reach cursor/agy by other paths.
The shim is accident-prevention rather than a security boundary, and closing these deliberate circumventions would need execve-level interception (LD_PRELOAD/seccomp) disproportionate to the raw hatch.
...
The fix is a native-identity-gated, debounced completion detector in the watcher's poll loop: `bin/fm-watch.sh`'s `maybe_native_turnend` reads the pane's actual native agent identity together with `agent_status`, requires that identity to match the recorded cursor/agy adapter, and, through `fm_transition_native_completion` (`bin/fm-transition-lib.sh`), touches the task's `state/<id>.turn-ended` when the matching pane settles at `idle`/`done` across two consecutive polls - the SAME completion signal every other harness's hook writes, so the existing `scan_signals`/wake path handles it with no change to the shared policy or the event stream.
