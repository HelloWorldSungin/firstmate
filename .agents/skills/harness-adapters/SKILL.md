...
description: >-
  ...
  Contains verified facts for claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, gemini, muse, rovo, and omp, plus the crew-only herdr-only agy adapter.
...
Agy is verified only for crewmate and scout work on the Herdr backend, never a primary, never a secondmate launcher, and never on another backend; `references/harness/agy.md` owns that boundary and its executable gates.
