...
`../../../bin/fm-launch-lib.sh` owns launch-command construction, including the autonomy flag and the model and effort flags.
`../../../bin/fm-spawn.sh` owns the surrounding per-task gates, task-kind compatibility, workspace-trust orchestration, and worker turn-end wiring.
...
Verify detection in `../../../bin/fm-harness.sh`, launch-command construction in `../../../bin/fm-launch-lib.sh` and its surrounding gates in `../../../bin/fm-spawn.sh`, busy state in `../../../bin/fm-busy-lib.sh`, shared composer behavior in `../../../bin/fm-composer-lib.sh`, lifecycle in `../../../bin/fm-control-lib.sh`, and tmux liveness in `../../../bin/backends/tmux.sh` when secondmate use is supported.
