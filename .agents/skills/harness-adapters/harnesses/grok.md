...
This is the observation that put the luminance half into the shared composer classifier, so grok needs no adapter-specific handling.
...
This is the observation that made the shared composer classifier's structural whole-box read necessary, so grok's cursor may sit on a content row or the bottom border without changing the result.
