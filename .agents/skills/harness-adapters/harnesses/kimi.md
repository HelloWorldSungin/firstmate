...
An early Enter can expand Kimi's composer to multiple content rows, leaving the pointer text on the first row and the cursor on an empty later row, which is the same single-cursor-row reading defect Grok's bottom-border quirk exposed and which the shared composer classifier's structural read already covers.
