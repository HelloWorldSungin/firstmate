...
- `BOARD_SWEEP: <project>: ... more items than the <n>-page cap reads`, `... more issues than the <n>-page cap reads`, `... stopped at its <n>-change limit`, or `... ran out of its <n>s whole-operation budget` - the sweep bounded itself and says what it left undone; the next sweep continues from where it stopped, so act only if the same line keeps returning, which means the bound is too small for that board rather than that one run was unlucky.
