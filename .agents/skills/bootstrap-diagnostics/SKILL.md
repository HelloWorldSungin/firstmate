...
- `GBRAIN_SERVING_CREDENTIAL: <detail>` - this home both serves its brain as the main brain and holds a usable hosted synthesis credential, the configuration the read-only share rule forbids, because since GBrain v0.42.76.0 a read-only holder reaches `think` on the serving home and the synthesis runs there under the serving home's model and credential.
  The boundary is the captain's to authorize, so report it plainly as a finding - the home can export its own brain content to a hosted provider at a reader's choosing - and let the captain decide the remediation: remove the hosted synthesis credential (and `think.secret`) from the serving home before relying on the share, or do not serve that brain.
  ...
  A line whose detail names an unreadable plane instead is `unknown` rather than a proven violation: the verdict could not be reached, which is still actionable because it is never reported as clean.
