...
- `BOARD_SWEEP: <project>: ... more items than the <n>-page cap reads` or `... stopped at its <n>-change limit` - the sweep bounded itself and says what it left; the next sweep continues, so act only if the same line keeps returning.
