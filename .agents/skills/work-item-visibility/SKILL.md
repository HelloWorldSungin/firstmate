...
`bin/fm-issue-comment.sh` refuses a note carrying those rather than publishing it, and a refusal is a rewrite, not a retry.
...
When reflecting reality would need a structural change - a status option that does not exist yet - `bin/fm-project-board.sh` reports it and leaves the board alone; raise it with the captain rather than adding it quietly.
