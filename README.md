...
- **Purpose-built task kinds** - ship tasks deliver authorized changes, scout tasks leave standalone investigation reports, and design tasks run a supervised interview that lands an ADR.
...
- `.agents/skills/` - agent-loaded skills (this section's table, firstmate's agent-only reference skills, and the vendored design toolkit that `bin/fm-spawn.sh` stages into a design task's worktree for the crewmate rather than loading in the first mate's own session). Every one of these assumes a live firstmate fleet and is meaningless, or actively misleading, installed anywhere else, so each carries `metadata.internal: true` in its frontmatter. That flag hides them from installer discovery (tools like the [skills.sh](https://skills.sh) `npx skills add` installer) without affecting how firstmate itself loads them - frontmatter metadata is inert to the agent's own skill loader.
