...
Pi's `/calm` toggle hides supported transcript chrome while retaining native working activity and all model context and session data.
Current Firstmate operational inputs remain visible ordinary user messages because Pi exposes no supported renderer for hiding them without changing their semantics.
