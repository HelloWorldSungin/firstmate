...
- **Read-only fleet dashboard** - an optional mobile-first view, loopback by default and reachable from off the machine only once you have configured authentication, renders the versioned fleet snapshot contract as six routed destinations - an age-sorted inbox of what needs you, a fleet board grouped by state, the read-only backlog queue, finished work with its rendered reports from the durable completion manifests that outlive cleanup, per-project token spend with the unattributed share kept visible, and searchable captured knowledge - each task one click from its own detail page, which - once you turn it on - carries a per-task timeline of safe lifecycle and tool summaries reported by the harnesses' own hooks, without owning dispatch, lifecycle, or fleet writes.
...
```
 ...
 └───┬────┘   └───┬────┘      └───┬────┘
  ...
  treehouse worktree, Orca worktree, or isolated secondmate home
     ...
     ├─ design: one-question interview ► ADR ► project mode ► teardown
     │
...
| `/updatefirstmate` | Fast-forward the running firstmate and its secondmates to the fork's latest, persist and restart every eligible live mate including already-current homes, report unconfirmed restarts honestly, and report upstream drift without merging it |
...
- [docs/usage-accounting.md](docs/usage-accounting.md) - collect token usage from Claude Code and Codex, keep it attributed to its task after cleanup and to the project it was spent on, and opt into cost estimates.
...
- [docs/verification/README.md](docs/verification/README.md) - append-only contract for dated maintainer-verification records.
