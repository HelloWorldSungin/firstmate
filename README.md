...
- **Read-only captain inbox, fleet board, completed-work history, and live agent activity** - an optional loopback-only, mobile-first view renders the versioned fleet snapshot contract as an age-sorted inbox of what needs you, a fleet health strip, and a kanban, browses finished work with its rendered reports from the durable completion manifests that outlive cleanup, and - once you turn it on - streams a per-agent timeline of safe lifecycle and tool summaries reported by the harnesses' own hooks, without owning dispatch, lifecycle, or fleet writes.
