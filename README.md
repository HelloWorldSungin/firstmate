...
- **Read-only fleet dashboard** - an optional mobile-first view, loopback by default and reachable from off the machine only once you have configured authentication, renders the versioned fleet snapshot contract as five routed destinations - an age-sorted inbox of what needs you, a fleet board grouped by state, the read-only backlog queue, finished work with its rendered reports from the durable completion manifests that outlive cleanup, and searchable captured knowledge - each task one click from its own detail page, which - once you turn it on - carries a per-task timeline of safe lifecycle and tool summaries reported by the harnesses' own hooks, without owning dispatch, lifecycle, or fleet writes.
