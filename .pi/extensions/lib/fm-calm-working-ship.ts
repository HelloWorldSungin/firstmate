...
// dispose(), and TUI.requestRender(); docs/calm-mode-feasibility.md owns which Pi
// declarations and CLI builds those are verified against.
// Pi renders a widget through Component.render(width), so this module recomputes its
// track from that width on every frame instead of caching a terminal size that a resize
// would invalidate. A resize while the boat is hidden is applied on the first resumed
// frame through the same clamp path.
